...
namespace Community.CsharpSqlite {
    ...
    *************************************************************************
    *///#include "sqliteInt.h"
		/* Forward references to internal structures *///typedef struct MemJournal MemJournal;
		...
		public class FileChunk {
			...
			/* Next chunk in the journal */public byte[] zChunk=new byte[JOURNAL_CHUNKSIZE];
		...
		public class FilePoint {
			public long iOffset;
			/* Offset from the beginning of the file */public FileChunk pChunk;
		...
		///
		///</summary>
		static int memjrnlRead(sqlite3_file pJfd,/* The journal file from which to read */byte[] zBuf,/* Put the results here */int iAmt,/* Number of bytes to read */sqlite3_int64 iOfst/* Begin reading at this offset */) {
			MemJournal p=(MemJournal)pJfd;
			byte[] zOut=zBuf;
			int nRead=iAmt;
			int iChunkOffset;
			FileChunk pChunk;
			/* SQLite never tries to read past the end of a rollback journal file */Debug.Assert(iOfst+iAmt<=p.endpoint.iOffset);
		...
		///
		///</summary>
		static int memjrnlWrite(sqlite3_file pJfd,/* The journal file into which to write */byte[] zBuf,/* Take data to be written from here */int iAmt,/* Number of bytes to write */sqlite3_int64 iOfst/* Begin writing at this offset into the file */) {
			MemJournal p=(MemJournal)pJfd;
			int nWrite=iAmt;
			byte[] zWrite=zBuf;
			int izWrite=0;
      ...
      */Debug.Assert(iOfst==p.endpoint.iOffset);
			...
			while(nWrite>0) {
				...
				if(iChunkOffset==0) {
					/* New chunk is required to extend the file. */FileChunk pNew=new FileChunk();
