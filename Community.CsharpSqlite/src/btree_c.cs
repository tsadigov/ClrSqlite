...
** on page 1, the transaction cannot be opened. */rc=p.querySharedCacheTableLock(MASTER_ROOT,READ_LOCK);
			if(SQLITE_OK!=rc)
				goto trans_begun;
...
*/static int sqlite3BtreeDelete(BtCursor pCur) {
			...
			/* Depth of node containing pCell */Debug.Assert(pCur.cursorHoldsMutex());
			Debug.Assert(pBt.inTransaction==TRANS_WRITE);
			Debug.Assert(!pBt.readOnly);
