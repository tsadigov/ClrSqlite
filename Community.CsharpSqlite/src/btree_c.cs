...
p.eState = CURSOR_INVALID;
...
#else
		/* Stub functions when INCRBLOB is omitted *///#define invalidateOverflowCache(x)
		static void invalidateOverflowCache(BtCursor pCur) {
		}
  ...
  */if(0==pCur.apPage[0].intKey) {
		...
		///</summary>
		static int btreeMoveto(BtCursor pCur,/* Cursor open on the btree to be searched */byte[] pKey,/* Packed key if the btree is an index */i64 nKey,/* Integer key for tables.  Size of pKey for indices */int bias,/* Bias search to the high end */ref int pRes/* Write search results here */) {
			int rc;
...
*/static int findOverflowCell(MemPage pPage,int iCell) {
		...
		static void btreeParseCellPtr(MemPage pPage,/* Page containing the cell */byte[] pCell,/* The actual data */ref CellInfo pInfo/* Fill in this structure */) {
			btreeParseCellPtr(pPage,pCell,0,ref pInfo);
		}
		static void btreeParseCellPtr(MemPage pPage,/* Page containing the cell */u8[] pCell,/* Pointer to the cell text. */int iCell,/* Pointer to the cell text. */ref CellInfo pInfo/* Fill in this structure */) {
			u16 n;
			/* Number bytes in cell content header */u32 nPayload=0;
			/* Number of bytes of cell payload */Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
    ...
    */int minLocal;
				...
				/* Overflow payload available for local storage */minLocal=pPage.minLocal;
		...
		static void btreeParseCell(MemPage pPage,/* Page containing the cell */int iCell,/* The cell index.  First cell is 0 */ref CellInfo pInfo/* Fill in this structure */) {
		...
		static u16 cellSizePtr(MemPage pPage,int iCell) {
			...
			if(iCell<0)
				...
				if(iCell>=pPage.aData.Length+1-pCell.Length)
...
static u16 cellSize( MemPage pPage, int iCell )
{
  return cellSizePtr( pPage, findCell( pPage, iCell ) );
		...
		static void ptrmapPutOvflPtr(MemPage pPage,int pCell,ref int pRC) {
			...
			if(info.iOverflow!=0) {
				Pgno ovfl=Converter.sqlite3Get4byte(pPage.aData,pCell,info.iOverflow);
		...
		static int defragmentPage(MemPage pPage) {
			...
			/* Last possible cell index */Debug.Assert(sqlite3PagerIswriteable(pPage.pDbPage));
...
*/if(pc<iCellFirst||pc>iCellLast) {
				    ...
				    if ( cbrk < iCellFirst || pc + size > usableSize )
    {
      return SQLITE_CORRUPT_BKPT();
		...
		static int allocateSpace(MemPage pPage,int nByte,ref int pIdx) {
			...
			/* Local cache of pPage.hdrOffset */u8[] data=pPage.aData;
			...
			/* Usable size of the page */Debug.Assert(sqlite3PagerIswriteable(pPage.pDbPage));
			Debug.Assert(pPage.pBt!=null);
			Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
			Debug.Assert(nByte>=0);
			/* Minimum cell size is 4 */Debug.Assert(pPage.nFree>=nByte);
			Debug.Assert(pPage.nOverflow==0);
			...
			testcase(gap==top);
			if(nFrag>=60) {
				/* Always defragment highly fragmented pages */rc=defragmentPage(pPage);
				if(rc!=0)
					return rc;
				top=get2byteNotZero(data,hdr+5);
			}
  ...
  */top-=nByte;
		...
		static int freeSpace(MemPage pPage,int start,int size) {
			...
			Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
			Debug.Assert(size>=0);
			/* Minimum cell size is 4 */if(pPage.pBt.secureDelete) {
  ...
  */hdr=pPage.hdrOffset;
		...
		static int decodeFlags(MemPage pPage,int flagByte) {
			BtShared pBt;
			/* A copy of pPage.pBt */Debug.Assert(pPage.hdrOffset==(pPage.pgno==1?100:0));
			Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
		...
		static int btreeInitPage(MemPage pPage) {
			...
			if(0==pPage.isInit) {
				...
				/* Last possible cell or freeblock offset */pBt=pPage.pBt;
				hdr=pPage.hdrOffset;
				data=pPage.aData;
    ...
    */if(nFree>usableSize) {
      ...
      ** the call for every page that comes in for re-initing. */btreeInitPage(pPage);
				}
			}
		}
		///<summary>
...
*/static int sqlite3BtreeOpen(sqlite3_vfs pVfs,/* VFS to use for this b-tree */string zFilename,/* Name of the file containing the BTree database */sqlite3 db,/* Associated database handle */ref Btree ppBtree,/* Pointer to new Btree object written here */int flags,/* Options */int vfsFlags/* Flags passed through to sqlite3_vfs.xOpen() */) {
			BtShared pBt=null;
			/* Shared part of btree structure */Btree p;
			/* Handle to return */sqlite3_mutex mutexOpen=null;
			/* Prevents a race condition. Ticket #3537 */int rc=SQLITE_OK;
			/* Result code from this function */u8 nReserve;
			/* Byte of unused space on each page */byte[] zDbHeader=new byte[100];
			/* Database header content *//* True if opening an ephemeral, temporary database */bool isTempDb=String.IsNullOrEmpty(zFilename);
  ...
  */
			...
			/* flags fit in 8 bits *//* Only a BTREE_SINGLE database can be BTREE_UNORDERED */Debug.Assert((flags&BTREE_UNORDERED)==0||(flags&BTREE_SINGLE)!=0);
			/* A BTREE_SINGLE database is always a temporary and/or ephemeral */Debug.Assert((flags&BTREE_SINGLE)==0||isTempDb);
...
*/static int sqlite3BtreeClose(ref Btree p) {
			BtShared pBt=p.pBt;
			BtCursor pCur;
			/* Close all cursors opened via this handle.  */Debug.Assert(sqlite3_mutex_held(p.db.mutex));
			sqlite3BtreeEnter(p);
			pCur=pBt.pCursor;
			while(pCur!=null) {
				BtCursor pTmp=pCur;
...
*/
		...
		static int sqlite3BtreeSetSafetyLevel(Btree p,/* The btree to set the safety level on */int level,/* PRAGMA synchronous.  1=OFF, 2=NORMAL, 3=FULL */int fullSync,/* PRAGMA fullfsync. */int ckptFullSync/* PRAGMA checkpoint_fullfync */) {
			BtShared pBt=p.pBt;
			Debug.Assert(sqlite3_mutex_held(p.db.mutex));
...
*/static int lockBtree(BtShared pBt) {
			int rc;
			/* Result code from subfunctions */MemPage pPage1=null;
			/* Page 1 of the database file */Pgno nPage;
			/* Number of pages in the database */Pgno nPageFile=0;
...
*/static int sqlite3BtreeBeginTrans(Btree p,int wrflag) {
  ...
  */if(p.inTrans==TRANS_WRITE||(p.inTrans==TRANS_READ&&0==wrflag)) {
			...
			/* Write transactions are not possible on a read-only database */if(pBt.readOnly&&wrflag!=0) {
				rc=SQLITE_READONLY;
				goto trans_begun;
			}
			#if !SQLITE_OMIT_SHARED_CACHE
...
** on page 1, the transaction cannot be opened. */rc=querySharedCacheTableLock(p,MASTER_ROOT,READ_LOCK);
			if(SQLITE_OK!=rc)
				goto trans_begun;
...
*/static int setChildPtrmaps(MemPage pPage) {
			...
			/* Return code */BtShared pBt=pPage.pBt;
			...
			for(i=0;i<nCell;i++) {
				...
				if(0==pPage.leaf) {
					Pgno childPgno=Converter.sqlite3Get4byte(pPage.aData,pCell);
					ptrmapPut(pBt,childPgno,PTRMAP_BTREE,pgno,ref rc);
				}
			}
			if(0==pPage.leaf) {
				Pgno childPgno=Converter.sqlite3Get4byte(pPage.aData,pPage.hdrOffset+8);
				ptrmapPut(pBt,childPgno,PTRMAP_BTREE,pgno,ref rc);
			}
...
*/static int modifyPagePointer(MemPage pPage,Pgno iFrom,Pgno iTo,u8 eType) {
			...
			if(eType==PTRMAP_OVERFLOW2) {
				/* The pointer is always the first 4 bytes of the page in this case.  */if(Converter.sqlite3Get4byte(pPage.aData)!=iFrom) {
					return SQLITE_CORRUPT_BKPT();
				}
				Converter.sqlite3Put4byte(pPage.aData,iTo);
			}
			else {
				u8 isInitOrig=pPage.isInit;
				int i;
				...
				for(i=0;i<nCell;i++) {
					...
					if(eType==PTRMAP_OVERFLOW1) {
						...
						btreeParseCellPtr(pPage,pCell,ref info);
						if(info.iOverflow!=0) {
							if(iFrom==Converter.sqlite3Get4byte(pPage.aData,pCell,info.iOverflow)) {
								Converter.sqlite3Put4byte(pPage.aData,pCell+info.iOverflow,(int)iTo);
								break;
							}
						}
					}
					else {
						if(Converter.sqlite3Get4byte(pPage.aData,pCell)==iFrom) {
							Converter.sqlite3Put4byte(pPage.aData,pCell,(int)iTo);
							break;
						}
					}
				}
				if(i==nCell) {
					if(eType!=PTRMAP_BTREE||Converter.sqlite3Get4byte(pPage.aData,pPage.hdrOffset+8)!=iFrom) {
						return SQLITE_CORRUPT_BKPT();
					}
					Converter.sqlite3Put4byte(pPage.aData,pPage.hdrOffset+8,iTo);
				}
				pPage.isInit=isInitOrig;
			}
			return SQLITE_OK;
...
** page.
*/static int relocatePage(BtShared pBt,/* Btree */MemPage pDbPage,/* Open page to move */u8 eType,/* Pointer map 'type' entry for pDbPage */Pgno iPtrPage,/* Pointer map 'page-no' entry for pDbPage */Pgno iFreePage,/* The location to move pDbPage to */int isCommit/* isCommit flag passed to sqlite3PagerMovepage */) {
			MemPage pPtrPage=new MemPage();
			/* The page that contains a pointer to pDbPage */Pgno iDbPage=pDbPage.pgno;
  ...
  */if(eType!=PTRMAP_ROOTPAGE) {
		...
		/* Forward declaration required by incrVacuumStep(). *///static int allocateBtreePage(BtShared *, MemPage **, Pgno *, Pgno, u8);
		/*
...
*/static int incrVacuumStep(BtShared pBt,Pgno nFin,Pgno iLastPg) {
			Pgno nFreeList;
			/* Number of pages still on the free-list */int rc;
			Debug.Assert(sqlite3_mutex_held(pBt.mutex));
...
*/static int autoVacuumCommit(BtShared pBt) {
			...
			if(!pBt.incrVacuum) {
				Pgno nFin;
				/* Number of pages in database after autovacuuming */Pgno nFree;
				/* Number of pages on the freelist initially */Pgno nPtrmap;
...
*/static void btreeEndTransaction(Btree p) {
    ...
    ** may still be reading from the database.  */downgradeAllSharedCacheTableLocks(p);
    ...
    ** call below will unlock the pager.  */if(p.inTrans!=TRANS_NONE) {
    ...
    ** pager if this call closed the only read or write transaction.  */p.inTrans=TRANS_NONE;
				unlockBtreeIfUnused(pBt);
			}
			btreeIntegrity(p);
		}
...
*/static int sqlite3BtreeSavepoint(Btree p,int op,int iSavepoint) {
      ...
      ** 28 is nonzero. */Debug.Assert(pBt.nPage>0);
				}
				sqlite3BtreeLeave(p);
			}
			return rc;
...
*/static int btreeCursor(Btree p,/* The btree */int iTable,/* Root page of table to open */int wrFlag,/* 1 to write. 0 read-only */KeyInfo pKeyInfo,/* First arg to comparison function */BtCursor pCur/* Space for new cursor */) {
			BtShared pBt=p.pBt;
			/* Shared b-tree handle */Debug.Assert(sqlite3BtreeHoldsMutex(p));
  ...
  ** this lock.  */Debug.Assert(hasSharedCacheTableLock(p,(u32)iTable,pKeyInfo!=null?1:0,wrFlag+1));
			Debug.Assert(wrFlag==0||!hasReadConflicts(p,(u32)iTable));
			/* Assert that the caller has opened the required transaction. */Debug.Assert(p.inTrans>TRANS_NONE);
  ...
  ** variables and link the cursor into the BtShared list.  */pCur.pgnoRoot=(Pgno)iTable;
			pCur.iPage=-1;
		...
		static int sqlite3BtreeCursor(Btree p,/* The btree */int iTable,/* Root page of table to open */int wrFlag,/* 1 to write. 0 read-only */KeyInfo pKeyInfo,/* First arg to xCompare() */BtCursor pCur/* Write new cursor here */) {
			int rc;
			sqlite3BtreeEnter(p);
...
*/static int sqlite3BtreeCloseCursor(BtCursor pCur) {
			...
			if(pBtree!=null) {
				...
				unlockBtreeIfUnused(pBt);
				invalidateOverflowCache(pCur);
				/* sqlite3_free(ref pCur); */sqlite3BtreeLeave(pBtree);
			}
			return SQLITE_OK;
		}
		/*
		...
		static void assertCellInfo( BtCursor pCur )
{
  ...
  Debug.Assert( info.GetHashCode() == pCur.info.GetHashCode() || info.Equals( pCur.info ) );//memcmp(info, pCur.info, sizeof(info))==0 );
		...
		#endif
		#if _MSC_VER
		/* Use a real function in MSVC to work around bugs in that compiler. */static void getCellInfo(BtCursor pCur) {
...
*/static int getOverflowPage(BtShared pBt,/* The database file */Pgno ovfl,/* Current overflow page number */out MemPage ppPage,/* OUT: MemPage handle (may be NULL) */out Pgno pPgnoNext/* OUT: Next overflow page number */) {
			Pgno next=0;
			MemPage pPage=null;
			ppPage=null;
			int rc=SQLITE_OK;
...
*/static int copyPayload(byte[] pPayload,/* Pointer to page data */u32 payloadOffset,/* Offset into page data */byte[] pBuf,/* Pointer to buffer */u32 pBufOffset,/* Offset into buffer */u32 nByte,/* Number of bytes to copy */int eOp,/* 0 . copy from page, 1 . copy to page */DbPage pDbPage/* Page containing pPayload */) {
			if(eOp!=0) {
				/* Copy data from buffer to page (a write operation) */int rc=sqlite3PagerWrite(pDbPage);
				if(rc!=SQLITE_OK) {
					return rc;
				}
				...
				/* Copy data from page to buffer (a read operation) */Buffer.BlockCopy(pPayload,(int)payloadOffset,pBuf,(int)pBufOffset,(int)nByte);
...
*/static int accessPayload(BtCursor pCur,/* Cursor pointing to entry to read from */u32 offset,/* Begin reading this far into payload */u32 amt,/* Read this many bytes */byte[] pBuf,/* Write the bytes into this buffer */int eOp/* zero to read. non-zero to write. */) {
			u32 pBufOffset=0;
			byte[] aPayload;
			int rc=SQLITE_OK;
			u32 nKey;
			...
			/* Btree page of current entry */BtShared pBt=pCur.pBt;
			/* Btree this cursor belongs to */Debug.Assert(pPage!=null);
			Debug.Assert(pCur.eState==CURSOR_VALID);
			...
			if(NEVER(offset+amt>nKey+pCur.info.nData)||pCur.info.nLocal>pBt.usableSize//&aPayload[pCur.info.nLocal] > &pPage.aData[pBt.usableSize]
				...
				/* Trying to read or write past the end of the data is an error */return SQLITE_CORRUPT_BKPT();
			}
			/* Check if data must be read/written to/from the btree page itself. */if(offset<pCur.info.nLocal) {
				int a=(int)amt;
...
*/static byte[] fetchPayload(BtCursor pCur,/* Cursor pointing to entry to read from */ref int pAmt,/* Write the number of available bytes here */ref int outOffset,/* Offset into Buffer */bool skipKey/* read beginning at data if this is true */) {
			byte[] aPayload;
			MemPage pPage;
			u32 nKey;
			u32 nLocal;
...
*/static int moveToRoot(BtCursor pCur) {
    ...
    ** return an SQLITE_CORRUPT error.  */Debug.Assert(pCur.apPage[0].intKey==1||pCur.apPage[0].intKey==0);
  ...
  ** to the page.  */pRoot=pCur.apPage[0];
...
**
*/static int sqlite3BtreeMovetoUnpacked(BtCursor pCur,/* The cursor to be moved */UnpackedRecord pIdxKey,/* Unpacked index key */i64 intKey,/* The table key */int biasRight,/* If true, bias the search to the high end */ref int pRes/* Write search results here */) {
			int rc;
			Debug.Assert(cursorHoldsMutex(pCur));
			Debug.Assert(sqlite3_mutex_held(pCur.pBtree.db.mutex));
  ...
  ** to move to, then just return without doing any work */if(pCur.eState==CURSOR_VALID&&pCur.validNKey&&pCur.apPage[0].intKey!=0) {
    ...
    ** a moveToChild() or moveToRoot() call would have detected corruption.  */Debug.Assert(pPage.nCell>0);
        ...
        */int nCell=pPage.aData[pCell+0];
          ...
          ** b-tree page.  */c=sqlite3VdbeRecordCompare(nCell,pPage.aData,pCell+1,pIdxKey);
          ...
          ** fits entirely on the main b-tree page.  */c=sqlite3VdbeRecordCompare(nCell,pPage.aData,pCell+2,pIdxKey);
          ...
          ** buffer before VdbeRecordCompare() can be called. */u8[] pCellKey;
...
*/static int allocateBtreePage(BtShared pBt,ref MemPage ppPage,ref Pgno pPgno,Pgno nearby,u8 exact) {
			...
			int rc;
			u32 n;
			/* Number of pages on the freelist */u32 k;
			/* Number of leaves on the trunk of the freelist */MemPage pTrunk=null;
			...
			if(n>0) {
				/* There are pages on the freelist.  Reuse one of those pages. */Pgno iTrunk;
				u8 searchList=0;
				/* If the free-list must be searched for 'nearby' *//* If the 'exact' parameter was true and a query of the pointer-map
...
*/rc=sqlite3PagerWrite(pPage1.pDbPage);
    ...
    */do {
					...
					/* # of leaves on this trunk page */if(k==0&&0==searchList) {
						/* The trunk has no leaves and the list is not being searched.
        ** So extract the trunk page itself and use it as the newly
        ** allocated page */Debug.Assert(pPrevTrunk==null);
						...
						if(k>(u32)(pBt.usableSize/4-2)) {
							/* Value of k is out of range.  Database corruption */rc=SQLITE_CORRUPT_BKPT();
							goto end_allocate_page;
							#if !SQLITE_OMIT_AUTOVACUUM
						}
						else
        ...
        */Debug.Assert(pPgno==iTrunk);
          ...
          */MemPage pNewTrunk=new MemPage();
								...
								if(k>0) {
									/* Extract a leaf from the trunk */u32 closest;
									Pgno iPage;
    ...
    ** end of the file */rc=sqlite3PagerWrite(pBt.pPage1.pDbPage);
				if(rc!=0)
					return rc;
				pBt.nPage++;
...
*/static int freePage2(BtShared pBt,MemPage pMemPage,Pgno iPage) {
			MemPage pTrunk=null;
			/* Free-list trunk page */Pgno iTrunk=0;
			/* Page number of free-list trunk page */MemPage pPage1=pBt.pPage1;
			/* Local reference to page 1 */MemPage pPage;
			/* Page being freed. May be NULL. */int rc;
			/* Return Code */int nFree;
			/* Initial number of pages on free-list */Debug.Assert(sqlite3_mutex_held(pBt.mutex));
...
** later.
*/static int fillInCell(MemPage pPage,/* The page that contains the cell */byte[] pCell,/* Complete text of the cell */byte[] pKey,i64 nKey,/* The key */byte[] pData,int nData,/* The data */int nZero,/* Extra zero bytes to append to pData */ref int pnSize/* Write cell size here */) {
			int nPayload;
			u8[] pSrc;
			int pSrcIndex=0;
			int nSrc,n,rc;
...
*/static void insertCell(MemPage pPage,/* Page into which we are copying */int i,/* New cell becomes the i-th cell of the page */u8[] pCell,/* Content of the new cell */int sz,/* Bytes of content in pCell */u8[] pTemp,/* Temp storage space for pCell, if needed */Pgno iChild,/* If non-zero, replace first 4 bytes with this value */ref int pRC/* Read and write return code from here */) {
  ...
  ** the term after the || in the following assert(). */Debug.Assert(sz==cellSizePtr(pPage,pCell)||(sz==8&&iChild>0));
...
*/static void assemblePage(MemPage pPage,/* The page to be assemblied */int nCell,/* The number of cells to add to this page */u8[] apCell,/* Pointer to a single the cell bodies */int[] aSize/* Sizes of the cells bodie*/) {
			int i;
			/* Loop counter */int pCellptr;
			/* Address of next cell pointer */int cellbody;
			/* Address of next cell body */byte[] data=pPage.aData;
			/* Pointer to data for pPage */int hdr=pPage.hdrOffset;
			/* Offset of header on pPage */int nUsable=(int)pPage.pBt.usableSize;
			/* Usable size of page */Debug.Assert(pPage.nOverflow==0);
			Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
			Debug.Assert(nCell>=0&&nCell<=(int)MX_CELL(pPage.pBt)&&(int)MX_CELL(pPage.pBt)<=10921);
			Debug.Assert(sqlite3PagerIswriteable(pPage.pDbPage));
			/* Check that the page has just been zeroed by zeroPage() */Debug.Assert(pPage.nCell==0);
			Debug.Assert(get2byteNotZero(data,hdr+5)==nUsable);
			pCellptr=pPage.cellOffset+nCell*2;
		...
		static void assemblePage(MemPage pPage,/* The page to be assemblied */int nCell,/* The number of cells to add to this page */u8[][] apCell,/* Pointers to cell bodies */u16[] aSize,/* Sizes of the cells */int offset/* Offset into the cell bodies, for c#  */) {
			int i;
			/* Loop counter */int pCellptr;
			/* Address of next cell pointer */int cellbody;
			/* Address of next cell body */byte[] data=pPage.aData;
		...
		static void assemblePage(MemPage pPage,/* The page to be assemblied */int nCell,/* The number of cells to add to this page */u8[] apCell,/* Pointers to cell bodies */u16[] aSize/* Sizes of the cells */) {
			int i;
			/* Loop counter */int pCellptr;
			/* Address of next cell pointer */int cellbody;
			/* Address of next cell body */u8[] data=pPage.aData;
			/* Pointer to data for pPage */int hdr=pPage.hdrOffset;
			/* Offset of header on pPage */int nUsable=(int)pPage.pBt.usableSize;
			/* Usable size of page */Debug.Assert(pPage.nOverflow==0);
			Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
...
*/static int NN=1;
		/* Number of neighbors on either side of pPage */static int NB=(NN*2+1);
...
*/static int balance_quick(MemPage pParent,MemPage pPage,u8[] pSpace) {
  ...
  */rc=allocateBtreePage(pBt,ref pNew,ref pgnoNew,0,0);
    ...
    */int iCell=findCell(pPage,pPage.nCell-1);
				...
				/* Insert the new divider cell into pParent. */insertCell(pParent,pParent.nCell,pSpace,pOut,//(int)(pOut-pSpace),
				null,pPage.pgno,ref rc);
				/* Set the right-child pointer of pParent to point to the new page. */Converter.sqlite3Put4byte(pParent.aData,pParent.hdrOffset+8,pgnoNew);
...
*/// under C#; Try to reuse Memory
		static int balance_nonroot(MemPage pParent,/* Parent page of siblings being balanced */int iParentIdx,/* Index of "the page" in pParent */u8[] aOvflSpace,/* page-size bytes of space for parent ovfl */int isRoot/* True if pParent is a root-page */) {
			...
			Pgno pgno;
			/* Temp var to store a page number in */pBt=pParent.pBt;
			Debug.Assert(sqlite3_mutex_held(pBt.mutex));
			Debug.Assert(sqlite3PagerIswriteable(pParent.pDbPage));
			#if FALSE
...
*/Debug.Assert(pParent.nOverflow==0||pParent.nOverflow==1);
  ...
  */i=pParent.nOverflow+pParent.nCell;
			...
			if((i+nxDiv-pParent.nOverflow)==pParent.nCell) {
				pRight=pParent.hdrOffset+8;
				//&pParent.aData[pParent.hdrOffset + 8];
			}
			else {
				pRight=findCell(pParent,i+nxDiv-pParent.nOverflow);
			}
			pgno=Converter.sqlite3Get4byte(pParent.aData,pRight);
			while(true) {
				...
				if(i+nxDiv==pParent.aOvfl[0].idx&&pParent.nOverflow!=0) {
					...
					pgno=Converter.sqlite3Get4byte(pParent.aOvfl[0].pCell,apDiv[i]);
					szNew[i]=cellSizePtr(pParent,apDiv[i]);
					pParent.nOverflow=0;
				}
				else {
					apDiv[i]=findCell(pParent,i+nxDiv-pParent.nOverflow);
					pgno=Converter.sqlite3Get4byte(pParent.aData,apDiv[i]);
					szNew[i]=cellSizePtr(pParent,apDiv[i]);
  ...
  */for(i=0;i<k-1;i++) {
			...
			Debug.Assert(sqlite3PagerIswriteable(pParent.pDbPage));
			Converter.sqlite3Put4byte(pParent.aData,pRight,apNew[nNew-1].pgno);
			/*
...
*/static int balance_deeper(MemPage pRoot,ref MemPage ppChild) {
			...
			/* Page number of the new child page */BtShared pBt=pRoot.pBt;
			/* The BTree */Debug.Assert(pRoot.nOverflow>0);
			Debug.Assert(sqlite3_mutex_held(pBt.mutex));
  ...
  */rc=sqlite3PagerWrite(pRoot.pDbPage);
			...
			/* Copy the overflow cells from pRoot to pChild */Array.Copy(pRoot.aOvfl,pChild.aOvfl,pRoot.nOverflow);
			//memcpy(pChild.aOvfl, pRoot.aOvfl, pRoot.nOverflow*sizeof(pRoot.aOvfl[0]));
			pChild.nOverflow=pRoot.nOverflow;
			/* Zero the contents of pRoot. Then install pChild as the right-child. */zeroPage(pRoot,pChild.aData[0]&~PTF_LEAF);
			Converter.sqlite3Put4byte(pRoot.aData,pRoot.hdrOffset+8,pgnoChild);
			ppChild=pChild;
			return SQLITE_OK;
		}
		/*
...
*/static u8[] aBalanceQuickSpace=new u8[13];
  ...
  int balance_deeper_called = 0;//TESTONLY( int balance_deeper_called = 0 );
        ...
        */Debug.Assert((balance_deeper_called++)==0);
          ...
          ** comes first. *///pFree = pSpace;
							}
						}
						pPage.nOverflow=0;
						/* The next iteration of the do-loop balances the parent page. */releasePage(pPage);
						pCur.iPage--;
					}
			}
			while(rc==SQLITE_OK);
...
*/static int sqlite3BtreeInsert(BtCursor pCur,/* Insert data into the table of this cursor */byte[] pKey,i64 nKey,/* The key of the new record */byte[] pData,int nData,/* The data of the new record */int nZero,/* Number of extra 0 bytes to append to data */int appendBias,/* True if this is likely an append */int seekResult/* Result of prior MovetoUnpacked() call */) {
			int rc;
			int loc=seekResult;
			/* -1: before desired location  +1: after */int szNew=0;
			int idx;
			MemPage pPage;
			Btree p=pCur.pBtree;
			BtShared pBt=p.pBt;
  ...
  ** blob of associated data.  */Debug.Assert((pKey==null)==(pCur.pKeyInfo==null));
  ...
  */pCur.info.nSize=0;
    ...
    ** from trying to save the current position of the cursor.  */pCur.apPage[pCur.iPage].nOverflow=0;
				pCur.eState=CURSOR_INVALID;
			}
...
*/static int sqlite3BtreeDelete(BtCursor pCur) {
			...
			BtShared pBt=p.pBt;
			int rc;
			/* Return code */MemPage pPage;
			/* Page to delete cell from */int pCell;
			/* Pointer to cell to delete */int iCellIdx;
			/* Index of cell to delete */int iCellDepth;
			/* Depth of node containing pCell */Debug.Assert(cursorHoldsMutex(pCur));
			Debug.Assert(pBt.inTransaction==TRANS_WRITE);
			Debug.Assert(!pBt.readOnly);
  ...
  ** balancing the tree following the delete operation easier.  */if(0==pPage.leaf) {
				int notUsed=0;
  ...
  ** node to replace the deleted cell.  */if(0==pPage.leaf) {
...
*/static int btreeCreateTable(Btree p,ref int piTable,int createTabFlags) {
			...
			int rc;
			int ptfFlags;
			/* Page-type flage for the root page of new table */Debug.Assert(sqlite3BtreeHoldsMutex(p));
			Debug.Assert(pBt.inTransaction==TRANS_WRITE);
			Debug.Assert(!pBt.readOnly);
			#if SQLITE_OMIT_AUTOVACUUM
...
return rc;
			...
			if(pBt.autoVacuum) {
				Pgno pgnoMove=0;
				/* Move a page here to make room for the root-page */MemPage pPageMove=new MemPage();
				/* The page to move to. *//* Creating a new table may probably require moving an existing database
...
*/static int clearDatabasePage(BtShared pBt,/* The BTree that contains the table */Pgno pgno,/* Page number to clear */int freePageFlag,/* Deallocate page if true */ref int pnChange/* Add number of Cells freed to this counter */) {
...
*/static int sqlite3BtreeClearTable(Btree p,int iTable,ref int pnChange) {
  ...
  ** a no-op).  */invalidateIncrblobCursors(p,0,1);
...
*/static int sqlite3BtreeCount(BtCursor pCur,ref i64 pnEntry) {
			i64 nEntry=0;
			/* Value to return in pnEntry */int rc;
			/* Return code */rc=moveToRoot(pCur);
			/* Unless an error occurs, the following loop runs one iteration for each
  ** page in the B-Tree structure (not including overflow pages).
  */while(rc==SQLITE_OK) {
				int iIdx;
				/* Index of child node in parent */MemPage pPage;
				/* Current page of the b-tree *//* If this is a leaf page or the tree is not an int-key tree, then
    ...
    */if(pPage.leaf!=0) {
					do {
						if(pCur.iPage==0) {
							/* All pages of the b-tree have been visited. Return successfully. */pnEntry=nEntry;
							return SQLITE_OK;
						}
						moveToParent(pCur);
					}
...
*/static void checkAppendMsg(IntegrityCk pCheck,string zMsg1,string zFormat,params object[] ap) {
			if(0==pCheck.mxErr)
				return;
			//va_list ap;
			lock(lock_va_list) {
		...
		static void checkAppendMsg(IntegrityCk pCheck,StringBuilder zMsg1,string zFormat,params object[] ap) {
			if(0==pCheck.mxErr)
				return;
			//va_list ap;
			lock(lock_va_list) {
...
** to pCheck.
*/static void checkPtrmap(IntegrityCk pCheck,/* Integrity check context */Pgno iChild,/* Child page number */u8 eType,/* Expected pointer map type */Pgno iParent,/* Expected pointer map parent page number */string zContext/* Context description (used for error msg) */) {
			int rc;
			u8 ePtrmapType=0;
...
*/static void checkList(IntegrityCk pCheck,/* Integrity checking context */int isFreeList,/* True for a freelist.  False for overflow page list */int iPage,/* Page number for first page in the list */int N,/* Expected number of pages in the list */string zContext/* Context for error messages */) {
			int i;
			int expected=N;
			int iFirst=iPage;
...
*/static i64 refNULL=0;
  ...
  */pBt=pCheck.pBt;
			...
			if((rc=btreeGetPage(pBt,(Pgno)iPage,ref pPage,0))!=0) {
				checkAppendMsg(pCheck,zContext.ToString(),"unable to get the page. error code=%d",rc);
				return 0;
			}
  ...
  ** btreeInitPage() is executed.  */pPage.isInit=0;
			if((rc=btreeInitPage(pPage))!=0) {
				...
				/* The only possible error from InitPage */checkAppendMsg(pCheck,zContext.ToString(),"btreeInitPage() returns error code %d",rc);
				releasePage(pPage);
				return 0;
			}
    ...
    */else
					if(i==0)
						...
						if(info.nKey<=nMaxKey) {
							checkAppendMsg(pCheck,zContext.ToString(),"Rowid %lld out of order (previous was %lld)",info.nKey,nMaxKey);
						}
						nMaxKey=info.nKey;
					}
			...
			//}else
			{
				...
				for(i=0;i<nCell;i++) {
					...
					if((int)(pc+size-1)>=usableSize) {
						checkAppendMsg(pCheck,"","Corruption detected in cell %d on page %d",i,iPage);
					}
					else {
				...
				for(i=cnt=0;i<usableSize;i++) {
					if(hit[i]==0) {
						...
						if(hit[i]>1) {
							checkAppendMsg(pCheck,"","Multiple uses for byte %d of page %d",i,iPage);
							break;
						}
				}
				if(cnt!=data[hdr+7]) {
					checkAppendMsg(pCheck,"","Fragmentation of %d bytes reported as %d on page %d",cnt,data[hdr+7],iPage);
				}
			}
...
*/static string sqlite3BtreeIntegrityCheck(Btree p,/* The btree to be checked */int[] aRoot,/* An array of root pages numbers for individual trees */int nRoot,/* Number of entries in aRoot[] */int mxErr,/* Stop reporting errors after this many */ref int pnErr/* Write number of errors seen to this variable */) {
			Pgno i;
			int nRef;
...
*/static int sqlite3BtreeSetVersion(Btree pBtree,int iVersion) {
			BtShared pBt=pBtree.pBt;
			int rc;
			/* Return code */Debug.Assert(pBtree.inTrans==TRANS_NONE);
