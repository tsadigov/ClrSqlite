...
namespace Community.CsharpSqlite {
    ...
    */static u8 sqlite3GetBoolean(string z) {
		...
		struct sPragmaType {
			public string zName;
			/* Name of the pragma */public int mask;
			/* Mask for the db.flags value */public sPragmaType(string zName,int mask) {
				this.zName=zName;
				this.mask=mask;
			}
		}
...
new sPragmaType( "vdbe_trace",               SQLITE_VdbeTrace     ),
				...
				#endif
				/* The following is VERY experimental */new sPragmaType("writable_schema",SQLITE_WriteSchema|SQLITE_RecoveryMode),
				new sPragmaType("omit_readlock",SQLITE_NoReadlock),
				/* TODO: Maybe it shouldn't be possible to change the ReadUncommitted
** flag if there are any active statements. */new sPragmaType("read_uncommitted",SQLITE_ReadUncommitted),
...
** are present in the build.  */
			...
			for(i=0;i<ArraySize(aPragma);i++)//, p++)
			 {
				...
				if(zLeft.Equals(p.zName,StringComparison.InvariantCultureIgnoreCase)) {
					...
					Debug.Assert(v!=null);
					/* Already allocated by sqlite3Pragma() */if(ALWAYS(v)) {
						if(null==zRight) {
							returnSingleInt(pParse,p.zName,((db.flags&p.mask)!=0)?1:0);
						}
						else {
							int mask=p.mask;
							/* Mask of bits to set or clear. */if(db.autoCommit==0) {
								/* Foreign key support may not be enabled or disabled while not
                ** in auto-commit mode.  */mask&=~(SQLITE_ForeignKeys);
							}
		...
		static EncName[] encnames=new EncName[] {
			...
			/* Must be element [1] */new EncName("UTF-16le (not supported)",SqliteEncoding.UTF16LE),
			/* Must be element [2] */new EncName("UTF-16be (not supported)",SqliteEncoding.UTF16BE),
			/* Must be element [3] */new EncName("UTF16le (not supported)",SqliteEncoding.UTF16LE),
			new EncName("UTF16be (not supported)",SqliteEncoding.UTF16BE),
			new EncName("UTF-16 (not supported)",0),
			/* SQLITE_UTF16NATIVE */new EncName("UTF16",0),
			/* SQLITE_UTF16NATIVE */new EncName(null,0)
		};
		...
		static void sqlite3Pragma(Parse pParse,Token pId1,/* First part of [database.]id field */Token pId2,/* Second part of [database.]id field, or NULL */Token pValue,/* Token for <value>, or NULL */int minusFlag/* True if a '-' sign preceded <value> */) {
			string zLeft=null;
			/* Nul-terminated UTF-8 string <id> */string zRight=null;
			/* Nul-terminated UTF-8 string <value>, or NULL */string zDb=null;
			/* The database name */Token pId=new Token();
			/* Pointer to <id> token */int iDb;
			/* Database index for <database> */sqlite3 db=pParse.db;
			Db pDb;
      ...
      ** index of the database this pragma is being applied to in db.aDb[]. */iDb=sqlite3TwoPartName(pParse,pId1,pId2,ref pId);
			if(iDb<0)
				return;
			pDb=db.aDb[iDb];
...
*/if(zLeft.Equals("default_cache_size",StringComparison.InvariantCultureIgnoreCase)) {
				VdbeOpList[] getCacheSize=new VdbeOpList[] {
					...
					/* 0 */new VdbeOpList(OP_ReadCookie,0,1,BTREE_DEFAULT_CACHE_SIZE),
					/* 1 */new VdbeOpList(OP_IfPos,1,7,0),
					...
					/* 6 */new VdbeOpList(OP_ResultRow,1,1,0),
				};
				int addr;
              ...
              */if(zLeft=="page_count") {
                ...
                */if(zLeft.Equals("locking_mode",StringComparison.InvariantCultureIgnoreCase)) {
                  ...
                  */if(zLeft=="journal_mode") {
										int eMode;
										/* One of the PAGER_JOURNALMODE_XXX symbols */int ii;
										/* Loop counter *//* Force the schema to be loaded on all databases.  This cases all
...
*/
                            ...
                            */int rc=sqlite3BtreeSetAutoVacuum(pBt,eAuto);
                              ...
                              */VdbeOpList[] setMeta6=new VdbeOpList[] {
																...
																/* 0 */new VdbeOpList(OP_ReadCookie,0,1,BTREE_LARGEST_ROOT_PAGE),
																/* 1 */new VdbeOpList(OP_If,1,0,0),
																/* 2 */new VdbeOpList(OP_Halt,SQLITE_OK,OE_Abort,0),
																/* 3 */new VdbeOpList(OP_Integer,0,1,0),
																/* 4 */new VdbeOpList(OP_SetCookie,0,BTREE_INCR_VACUUM,1),
															/* 5 */};
															int iAddr;
																				...
																				for(i=0;i<pTab.nCol;i++)//, pCol++)
																				 {
																					...
																					sqlite3VdbeAddOp4(v,OP_String8,0,3,0,pCol.zType!=null?pCol.zType:"",0);
																				...
																				if(zLeft.Equals("index_list",StringComparison.InvariantCultureIgnoreCase)&&zRight!=null) {
																					...
																					if(zLeft.Equals("database_list",StringComparison.InvariantCultureIgnoreCase)) {
																						...
																						for(i=0;i<db.nDb;i++) {
																							...
																							sqlite3VdbeAddOp4(v,OP_String8,0,3,0,sqlite3BtreeGetFilename(db.aDb[i].pBt),0);
																							sqlite3VdbeAddOp2(v,OP_ResultRow,1,3);
																						}
																					}
																					else
																									...
																									if(pFK!=null) {
																										...
																										while(pFK!=null) {
																											...
																											for(j=0;j<pFK.nCol;j++) {
																												...
																												sqlite3VdbeAddOp4(v,OP_String8,0,4,0,pTab.aCol[pFK.aCol[j].iFrom].zName,0);
...
*/if(zLeft.Equals("integrity_check",StringComparison.InvariantCultureIgnoreCase)||zLeft.Equals("quick_check",StringComparison.InvariantCultureIgnoreCase)) {
                                                      ...
                                                      */VdbeOpList[] endCode=new VdbeOpList[] {
																											...
																											/* 0 */new VdbeOpList(OP_IfNeg,1,0,0),
																											/* 1 */new VdbeOpList(OP_String8,0,3,0),
																											/* 2 */new VdbeOpList(OP_ResultRow,3,1,0),
																										};
																										...
																										/* reg[1] holds errors left *//* Do an integrity check on each database file */for(i=0;i<db.nDb;i++) {
																											HashElem x;
																											Hash pTbls;
																											int cnt=0;
																											...
																											/* Halt if out of errors */sqlite3VdbeAddOp2(v,OP_Halt,0,0);
																											sqlite3VdbeJumpHere(v,addr);
                                                        ...
                                                        */Debug.Assert(sqlite3SchemaMutexHeld(db,iDb,null));
																											...
																											/* Make sure sufficient number of registers have been allocated */if(pParse.nMem<cnt+4) {
																												pParse.nMem=cnt+4;
																											}
																											/* Do the b-tree integrity checks */sqlite3VdbeAddOp3(v,OP_IntegrityCk,2,cnt,1);
                                                        ...
                                                        */for(x=pTbls.first;x!=null&&!isQuick;x=x.next) {
																												...
																												if(pTab.pIndex==null)
																													continue;
																												addr=sqlite3VdbeAddOp1(v,OP_IfPos,1);
																												/* Stop if out of errors */sqlite3VdbeAddOp2(v,OP_Halt,0,0);
																												sqlite3VdbeJumpHere(v,addr);
																												sqlite3OpenTableAndIndices(pParse,pTab,1,OP_OpenRead);
																												sqlite3VdbeAddOp2(v,OP_Integer,0,2);
																												/* reg(2) will count entries */loopTop=sqlite3VdbeAddOp2(v,OP_Rewind,1,0);
																												sqlite3VdbeAddOp2(v,OP_AddImm,2,1);
																												/* increment entry count */for(j=0,pIdx=pTab.pIndex;pIdx!=null;pIdx=pIdx.pNext,j++) {
																													int jmp2;
																													int r1;
																													VdbeOpList[] idxErr=new VdbeOpList[] {
																														new VdbeOpList(OP_AddImm,1,-1,0),
																														new VdbeOpList(OP_String8,0,3,0),
																														/* 1 */new VdbeOpList(OP_Rowid,1,4,0),
																														new VdbeOpList(OP_String8,0,5,0),
																														/* 3 */new VdbeOpList(OP_String8,0,6,0),
																														/* 4 */new VdbeOpList(OP_Concat,4,3,3),
																												...
																												for(j=0,pIdx=pTab.pIndex;pIdx!=null;pIdx=pIdx.pNext,j++) {
																													VdbeOpList[] cntIdx=new VdbeOpList[] {
																														...
																														/* 1 */new VdbeOpList(OP_AddImm,3,1,0),
																														new VdbeOpList(OP_Next,0,0,0),
																														/* 3 */new VdbeOpList(OP_Eq,2,0,3),
																														/* 4 */new VdbeOpList(OP_AddImm,1,-1,0),
																														new VdbeOpList(OP_String8,0,2,0),
																														/* 6 */new VdbeOpList(OP_String8,0,3,0),
																														/* 7 */new VdbeOpList(OP_Concat,3,2,2),
...
*/if(zLeft.Equals("encoding",StringComparison.InvariantCultureIgnoreCase)) {
																											int iEnc;
																											if(null==zRight) {
																												/* "PRAGMA encoding" */if(sqlite3ReadSchema(pParse)!=0) {
																													pParse.nErr=0;
																													pParse.zErrMsg=null;
																													pParse.rc=0;
...
*/if(zLeft.Equals("schema_version",StringComparison.InvariantCultureIgnoreCase)||zLeft.Equals("user_version",StringComparison.InvariantCultureIgnoreCase)||zLeft.Equals("freelist_count",StringComparison.InvariantCultureIgnoreCase)) {
																												int iCookie;
																												/* Cookie index. 1 for schema-cookie, 6 for user-cookie. */sqlite3VdbeUsesBtree(v,iDb);
																												switch(zLeft[0]) {
																												case 'f':
																												case 'F':
																												...
																												if(zRight!=null&&iCookie!=BTREE_FREE_PAGE_COUNT) {
																													/* Write the specified cookie value */VdbeOpList[] setCookie=new VdbeOpList[] {
																														new VdbeOpList(OP_Transaction,0,1,0),
																														/* 0 */new VdbeOpList(OP_Integer,0,1,0),
																														/* 1 */new VdbeOpList(OP_SetCookie,0,0,1),
																													/* 2 */};
																													...
																													/* Read the specified cookie value */VdbeOpList[] readCookie=new VdbeOpList[] {
																														new VdbeOpList(OP_Transaction,0,0,0),
																														/* 0 */new VdbeOpList(OP_ReadCookie,0,1,0),
																														/* 1 */new VdbeOpList(OP_ResultRow,1,1,0)
																													};
																												...
																												if(zLeft.Equals("reload_schema",StringComparison.InvariantCultureIgnoreCase)) {
																													/* force schema reloading*/sqlite3ResetInternalSchema(db,-1);
																												}
																												else
...
sqlite3_activate_cerod(&zRight[6]);
      ...
      */
			#if !SQLITE_OMIT_PAGER_PRAGMAS
			if(db.autoCommit!=0) {
				sqlite3BtreeSetSafetyLevel(pDb.pBt,pDb.safety_level,((db.flags&SQLITE_FullFSync)!=0)?1:0,((db.flags&SQLITE_CkptFullFSync)!=0)?1:0);
			}
			#endif
			pragma_out:
