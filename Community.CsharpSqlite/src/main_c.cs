...
*/if(sqlite3GlobalConfig.isInit!=0)
      ...
      ** the pInitMutex mutex. Return an error in either case.  */if(rc!=SQLITE_OK) {
				return rc;
			}
    ...
    */public static int sqlite3_shutdown() {
		...
		static int sqlite3_config(int op,sqlite3_pcache_methods ap) {
			...
			case SQLITE_CONFIG_PCACHE: {
				/* Specify an alternative malloc implementation */sqlite3GlobalConfig.pcache=ap;
		...
		static int sqlite3_config(int op,sqlite3_mem_methods ap) {
			...
			case SQLITE_CONFIG_MALLOC: {
				/* Specify an alternative malloc implementation */sqlite3GlobalConfig.m=ap;
		...
		static int sqlite3_config(int op,ref sqlite3_mem_methods ap) {
			...
			case SQLITE_CONFIG_GETMALLOC: {
				/* Retrieve the current malloc() implementation *///if ( sqlite3GlobalConfig.m.xMalloc == null ) sqlite3MemSetDefault();
    ...
    static int sqlite3_config( int op, ref sqlite3_mutex_methods ap )
    {
      ...
      ** the SQLite library is in use. */if(sqlite3GlobalConfig.isInit!=0)
          ...
          case SQLITE_CONFIG_GETMUTEX:
            {
              ...
              break;
				...
				case SQLITE_CONFIG_MALLOC: {
					Debugger.Break();
					// TODO --
					/* Specify an alternative malloc implementation */sqlite3GlobalConfig.m=va_arg(ap,(sqlite3_mem_methods)null);
					break;
				}
				case SQLITE_CONFIG_GETMALLOC: {
					/* Retrieve the current malloc() implementation *///if ( sqlite3GlobalConfig.m.xMalloc == null ) sqlite3MemSetDefault();
				...
				case SQLITE_CONFIG_MEMSTATUS: {
					/* Enable or disable the malloc status collection */sqlite3GlobalConfig.bMemstat=va_arg(ap,(Int32)0)!=0;
					break;
				}
				case SQLITE_CONFIG_SCRATCH: {
					/* Designate a buffer for scratch memory space */sqlite3GlobalConfig.pScratch=va_arg(ap,(Byte[][])null);
					sqlite3GlobalConfig.szScratch=va_arg(ap,(Int32)0);
					sqlite3GlobalConfig.nScratch=va_arg(ap,(Int32)0);
					break;
				}
				case SQLITE_CONFIG_PAGECACHE: {
					/* Designate a buffer for page cache memory space */sqlite3GlobalConfig.pPage=va_arg(ap,(MemPage)null);
					sqlite3GlobalConfig.szPage=va_arg(ap,(Int32)0);
					sqlite3GlobalConfig.nPage=va_arg(ap,(Int32)0);
					break;
				}
				case SQLITE_CONFIG_PCACHE: {
					/* Specify an alternative page cache implementation */Debugger.Break();
...
break;
		...
		public class _aFlagOp {
			public int op;
			/* The opcode */public u32 mask;
			/* Mask of the bit in sqlite3.flags to set/clear */public _aFlagOp(int op,u32 mask) {
				this.op=op;
				this.mask=mask;
			}
		}
		...
		static int sqlite3_db_config(sqlite3 db,int op,params object[] ap) {
			...
			lock(lock_va_list) {
				...
				case SQLITE_DBCONFIG_LOOKASIDE: {
					...
					/* IMP: R-26835-10964 */int sz=va_arg(ap,(Int32)0);
					/* IMP: R-47871-25994 */int cnt=va_arg(ap,(Int32)0);
					/* IMP: R-04460-53386 */rc=setupLookaside(db,pBuf,sz,cnt);
					break;
				}
				default: {
					...
					uint i;
					rc=SQLITE_ERROR;
					/* IMP: R-42790-23372 */for(i=0;i<ArraySize(aFlagOp);i++) {
						if(aFlagOp[i].op==op) {
		...
		public static int sqlite3_close(sqlite3 db) {
			...
			/* Force xDestroy calls on all virtual tables */sqlite3ResetInternalSchema(db,-1);
      ...
      */sqlite3VtabRollback(db);
			/* If there are any outstanding VMs, return SQLITE_BUSY. */if(db.pVdbe!=null) {
			...
			/* Free any outstanding Savepoint structures. */sqlite3CloseSavepoints(db);
			for(j=0;j<db.nDb;j++) {
				Db pDb=db.aDb[j];
				if(pDb.pBt!=null) {
      ...
      */sqlite3ConnectionClosed(db);
			...
			for(i=db.aCollSeq.first;i!=null;i=i.next) {
				...
				// sqliteHashData(i);
				/* Invoke any destructors registered for collation sequence user data. */for(j=0;j<3;j++) {
			...
			/* Deallocates any cached error strings. */if(db.pErr!=null) {
      ...
      */sqlite3DbFree(db,ref db.aDb[1].pSchema);
			...
			/* Fails on a lookaside memory leak *///if ( db.lookaside.bMalloced )
			//{
		...
		static void sqlite3RollbackAll(sqlite3 db) {
			...
			/* Any deferred constraint violations have now been resolved. */db.nDeferredCons=0;
			/* If one has been configured, invoke the rollback-hook callback */if(db.xRollbackCallback!=null&&(inTrans!=0||0==db.autoCommit)) {
		...
		///
		///</summary>
		static int sqliteDefaultBusyCallback(object ptr,/* Database connection */int count/* Number of times table has been busy */) {
...
*/p=sqlite3FindFunction(db,zFunctionName,nName,nArg,enc,0);
      ...
      ** being replaced invoke the destructor function here. */functionDestroy(db,p);
			if(pDestructor!=null) {
				pDestructor.nRef++;
			}
...
return rc;
		...
		/// rollback.
		///</summary>
		static object sqlite3_commit_hook(sqlite3 db,/* Attach the hook to this database */dxCommitCallback xCallback,//int (*xCallback)(void),  /* Function to invoke on each commit */
		...
		///
		///</summary>
		static object sqlite3_update_hook(sqlite3 db,/* Attach the hook to this database */dxUpdateCallback xCallback,//void (*xCallback)(void*,int,char const *,char const *,sqlite_int64),
		object pArg/* Argument to the function */) {
			object pRet;
			sqlite3_mutex_enter(db.mutex);
			pRet=db.pUpdateArg;
		...
		///
		///</summary>
		static object sqlite3_rollback_hook(sqlite3 db,/* Attach the hook to this database */dxRollbackCallback xCallback,//void (*xCallback)(void), /* Callback function */
		object pArg/* Argument to the function */) {
			object pRet;
			sqlite3_mutex_enter(db.mutex);
...
sqlite3_wal_hook(db, 0, 0);
		...
		///
		///</summary>
		static object sqlite3_wal_hook(sqlite3 db,/* Attach the hook to this db handle */dxWalCallback xCallback,//int(*xCallback)(void *, sqlite3*, const char*, int),
...
return pRet;
		...
		///
		///</summary>
		static int sqlite3_wal_checkpoint_v2(sqlite3 db,/* Database handle */string zDb,/* Name of attached database (or NULL) */int eMode,/* SQLITE_CHECKPOINT_* value */out int pnLog,/* OUT: Size of WAL log in frames */out int pnCkpt/* OUT: Total number of frames checkpointed */) {
			#if SQLITE_OMIT_WAL
			pnLog=0;
			pnCkpt=0;
			return SQLITE_OK;
...
*/static int sqlite3ParseUri(string zDefaultVfs,/* VFS to use if no "vfs=xxx" query option */string zUri,/* Nul-terminated URI to parse */ref int pFlags,/* IN/OUT: SQLITE_OPEN_XXX flags */ref sqlite3_vfs ppVfs,/* OUT: VFS to use */ref string pzFile,/* OUT: Filename component of URI */ref string pzErrMsg/* OUT: Error message (if rc!=SQLITE_OK) */) {
			...
			if(((flags&SQLITE_OPEN_URI)!=0||sqlite3GlobalConfig.bOpenUri)&&nUri>=5&&memcmp(zUri,"file:",5)==0) {
				string zOpt;
				int eState;
				/* Parser state when parsing URI */int iIn;
				/* Input character index *///int iOut = 0;                 /* Output character index */
				int nByte=nUri+2;
				/* Bytes of space to allocate *//* Make sure the SQLITE_OPEN_URI flag is set to indicate to the VFS xOpen 
    ** method that there may be extra parameters following the file-name.  */flags|=SQLITE_OPEN_URI;
				for(iIn=0;iIn<nUri;iIn++)
					nByte+=(zUri[iIn]=='&')?1:0;
				//zFile = sqlite3_malloc(nByte);
				//if( null==zFile ) return SQLITE_NOMEM;
				zFile=new StringBuilder(nByte);
				/* Discard the scheme and authority segments of the URI. */if(zUri[5]=='/'&&zUri[6]=='/') {
					iIn=7;
    ...
    */eState=0;
          ...
          ** and skip to the next "?", "=" or "&", as appropriate. */while(iIn<nUri&&(c=zUri[iIn])!=0&&c!='#'&&(eState!=0||c!='?')&&(eState!=1||(c!='='&&c!='&'))&&(eState!=2||c!='&')) {
								iIn++;
							}
							continue;
						}
						...
						if(eState==1&&(c=='&'||c=='=')) {
							if(zFile[zFile.Length-1]=='\0') {
								/* An empty option name. Ignore this option altogether. */while(zUri[iIn]!='\0'&&zUri[iIn]!='#'&&zUri[iIn-1]!='&')
									iIn++;
								continue;
							}
							if(c=='&') {
    ...
    ** method. */zOpt=zFile.ToString().Substring(StringExtensions.sqlite3Strlen30(zFile)+1);
				while(zOpt.Length>0) {
    ...
    ** is UTF-8 encoded.
    */static int openDatabase(string zFilename,/* Database filename UTF-8 encoded */out sqlite3 ppDb,/* OUT: Returned database handle */int flags,/* Operational flags */string zVfs/* Name of the VFS to use */) {
			sqlite3 db;
			/* Store allocated handle here */int rc;
			/* Return code */int isThreadsafe;
			/* True for threadsafe connections */string zOpen="";
			/* Filename argument to pass to BtreeOpen() */string zErrMsg="";
			/* Error message from sqlite3ParseUri() */ppDb=null;
...
*/Debug.Assert(SQLITE_OPEN_READONLY==0x01);
			...
			/* READONLY */testcase((1<<(flags&7))==0x04);
			/* READWRITE */testcase((1<<(flags&7))==0x40);
			/* READWRITE | CREATE */if(((1<<(flags&7))&0x46)==0)
				return SQLITE_MISUSE_BKPT();
      ...
      */flags&=~(SQLITE_OPEN_DELETEONCLOSE|SQLITE_OPEN_EXCLUSIVE|SQLITE_OPEN_MAIN_DB|SQLITE_OPEN_TEMP_DB|SQLITE_OPEN_TRANSIENT_DB|SQLITE_OPEN_MAIN_JOURNAL|SQLITE_OPEN_TEMP_JOURNAL|SQLITE_OPEN_SUBJOURNAL|SQLITE_OPEN_MASTER_JOURNAL|SQLITE_OPEN_NOMUTEX|SQLITE_OPEN_FULLMUTEX|SQLITE_OPEN_WAL);
			/* Allocate the sqlite data structure */db=new sqlite3();
      ...
      */createCollation(db,"BINARY",SqliteEncoding.UTF8,CollationType.BINARY,0,binCollFunc,null);
			...
			/* Also add a UTF-8 case-insensitive collation sequence. */createCollation(db,"NOCASE",SqliteEncoding.UTF8,CollationType.NOCASE,0,nocaseCollatingFunc,null);
			/* Parse the filename/URI argument. */db.openFlags=flags;
			...
			/* Open the backend database driver */rc=sqlite3BtreeOpen(db.pVfs,zOpen,db,ref db.aDb[0].pBt,0,flags|SQLITE_OPEN_MAIN_DB);
			if(rc!=SQLITE_OK) {
...
SQLITE_DEFAULT_LOCKING_MODE);
#endif
			/* Enable the lookaside-malloc subsystem */setupLookaside(db,null,sqlite3GlobalConfig.szLookaside,sqlite3GlobalConfig.nLookaside);
    ...
    */static public int sqlite3_open(string zFilename,out sqlite3 ppDb) {
		...
		static public int sqlite3_open_v2(string filename,/* Database filename (UTF-8) */out sqlite3 ppDb,/* OUT: SQLite db handle */int flags,/* Flags */string zVfs/* Name of VFS module to use */) {
