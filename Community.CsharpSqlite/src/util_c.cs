...
namespace Community.CsharpSqlite {
	...
	public partial class StringExtensions {
		...
		public static int sqlite3Dequote(ref string z) {
			...
			case '`':
			break;
			/* For MySQL compatibility */case '[':
			quote=']';
			break;
			/* For MS SqlServer compatibility */default:
			return -1;
			}
...
dummy += (uint)x;
    ...
    */public static int compare2pow63(string zNum,int incr) {
			int c=0;
			int i;
			/* 012345678901234567 */string pow63="922337203685477580";
			for(i=0;c==0&&i<18;i++) {
		...
		static u8 sqlite3GetVarint(byte[] p,int offset,out u64 v) {
			u32 a,b,s;
			a=p[offset+0];
			/* a: p0 (unmasked) */if(0==(a&0x80)) {
				v=a;
				return 1;
			}
			//p++;
			b=p[offset+1];
			/* b: p1 (unmasked) */if(0==(b&0x80)) {
				a&=0x7f;
				a=a<<7;
				a|=b;
				v=a;
			...
			/* Verify that constants are precomputed correctly */Debug.Assert(SLOT_2_0==((0x7f<<14)|(0x7f)));
			Debug.Assert(SLOT_4_2_0==((0xfU<<28)|(0x7f<<14)|(0x7f)));
			//p++;
			a=a<<14;
			a|=p[offset+2];
			/* a: p0<<14 | p2 (unmasked) */if(0==(a&0x80)) {
				a&=SLOT_2_0;
				b&=0x7f;
				b=b<<7;
				a|=b;
			...
			/* CSE1 from below */a&=SLOT_2_0;
			//p++;
			b=b<<14;
			b|=p[offset+3];
			/* b: p1<<14 | p3 (unmasked) */if(0==(b&0x80)) {
				b&=SLOT_2_0;
				/* moved CSE1 up *//* a &= (0x7f<<14)|(0x7f); */a=a<<7;
				a|=b;
				v=a;
				return 4;
			}
			/* a: p0<<14 | p2 (masked) *//* b: p1<<14 | p3 (unmasked) *//* 1:save off p0<<21 | p1<<14 | p2<<7 | p3 (masked) *//* moved CSE1 up *//* a &= (0x7f<<14)|(0x7f); */b&=SLOT_2_0;
			s=a;
			/* s: p0<<14 | p2 (masked) *///p++;
			a=a<<14;
			a|=p[offset+4];
			/* a: p0<<28 | p2<<14 | p4 (unmasked) */if(0==(a&0x80)) {
				/* we can skip these cause they were (effectively) done above in calc'ing s *//* a &= (0x1f<<28)|(0x7f<<14)|(0x7f); *//* b &= (0x7f<<14)|(0x7f); */b=b<<7;
				a|=b;
				s=s>>18;
				v=((u64)s)<<32|a;
				return 5;
			}
			/* 2:save off p0<<21 | p1<<14 | p2<<7 | p3 (masked) */s=s<<7;
			s|=b;
			/* s: p0<<21 | p1<<14 | p2<<7 | p3 (masked) *///p++;
			b=b<<14;
			b|=p[offset+5];
			/* b: p1<<28 | p3<<14 | p5 (unmasked) */if(0==(b&0x80)) {
				/* we can skip this cause it was (effectively) done above in calc'ing s *//* b &= (0x1f<<28)|(0x7f<<14)|(0x7f); */a&=SLOT_2_0;
				a=a<<7;
				a|=b;
				s=s>>18;
				v=((u64)s)<<32|a;
			...
			a=a<<14;
			a|=p[offset+6];
			/* a: p2<<28 | p4<<14 | p6 (unmasked) */if(0==(a&0x80)) {
				a&=SLOT_4_2_0;
				b&=SLOT_2_0;
				b=b<<7;
				a|=b;
			...
			/* CSE2 from below */a&=SLOT_2_0;
			//p++;
			b=b<<14;
			b|=p[offset+7];
			/* b: p3<<28 | p5<<14 | p7 (unmasked) */if(0==(b&0x80)) {
				b&=SLOT_4_2_0;
				/* moved CSE2 up *//* a &= (0x7f<<14)|(0x7f); */a=a<<7;
				a|=b;
				s=s>>4;
				v=((u64)s)<<32|a;
				return 8;
			...
			a=a<<15;
			a|=p[offset+8];
			/* a: p4<<29 | p6<<15 | p8 (unmasked) *//* moved CSE2 up *//* a &= (0x7f<<29)|(0x7f<<15)|(0xff); */b&=SLOT_2_0;
			b=b<<8;
			a|=b;
			s=s<<4;
			b=p[offset+4];
      ...
      ** by the getVarin32() macro */a=p[offset+0];
			/* a: p0 (unmasked) *///#if getVarint32
			//  if ( 0==( a&0x80))
			//  {
			/* Values between 0 and 127 *///    v = a;
			//    return 1;
			//  }
			//#endif
			/* The 2-byte case *///p++;
			...
			/* b: p1 (unmasked) */if(0==(b&0x80)) {
				/* Values between 128 and 16383 */a&=0x7f;
				a=a<<7;
				v=a|b;
				return 2;
			}
			...
			/* a: p0<<14 | p2 (unmasked) */if(0==(a&0x80)) {
				/* Values between 16384 and 2097151 */a&=(0x7f<<14)|(0x7f);
				b&=0x7f;
				b=b<<7;
				v=a|b;
				return 3;
	...
	public partial class Converter {
        ...
        */testcase(i==10);
		...
		public static bool sqlite3AtoF(string z,ref double pResult,int length,SqliteEncoding enc) {
			...
			/* sign * significand * (10 ^ (esign * exponent)) */int sign=1;
			/* sign of significand */i64 s=0;
			/* significand */int d=0;
			/* adjust exponent for shifting decimal point */int esign=1;
			/* sign of exponent */int e=0;
			/* exponent */int eValid=1;
			/* True exponent is either not used or is well-formed */double result=0;
			int nDigits=0;
			pResult=0.0;
			/* Default return value, in case of an error */int zDx=0;
			...
			if(zDx>=length)
				return false;
			/* get sign of significand */if(z[zDx]=='-') {
				sign=-1;
				zDx+=incr;
			}
			else
			...
			/* skip leading zeroes */while(zDx<z.Length&&z[zDx]=='0') {
				zDx+=incr;
				nDigits++;
			}
			/* copy max significant digits to significand */while(zDx<length&&CharExtensions.sqlite3Isdigit(z[zDx])&&s<((IntegerExtensions.LARGEST_INT64-9)/10)) {
				s=s*10+(z[zDx]-'0');
				zDx+=incr;
				nDigits++;
			}
			/* skip non-significant significand digits
      ** (increase exponent by d to shift decimal left) */while(zDx<length&&CharExtensions.sqlite3Isdigit(z[zDx])) {
				zDx+=incr;
				nDigits++;
				d++;
			}
			if(zDx>=length)
				goto do_atof_calc;
			/* if decimal point is present */if(z[zDx]=='.') {
				zDx+=incr;
				/* copy digits from after decimal to significand
        ** (decrease exponent by d to shift decimal right) */while(zDx<length&&CharExtensions.sqlite3Isdigit(z[zDx])&&s<((IntegerExtensions.LARGEST_INT64-9)/10)) {
					s=s*10+(z[zDx]-'0');
					zDx+=incr;
					nDigits++;
					d--;
				}
				/* skip non-significant digits */while(zDx<length&&CharExtensions.sqlite3Isdigit(z[zDx])) {
					zDx+=incr;
					nDigits++;
				}
				if(zDx>=length)
			...
			/* if exponent is present */if(z[zDx]=='e'||z[zDx]=='E') {
				zDx+=incr;
				eValid=0;
				if(zDx>=length)
					goto do_atof_calc;
				/* get sign of exponent */if(z[zDx]=='-') {
					esign=-1;
					zDx+=incr;
				}
				else
				...
				/* copy digits to exponent */while(zDx<length&&CharExtensions.sqlite3Isdigit(z[zDx])) {
					e=e*10+(z[zDx]-'0');
					zDx+=incr;
					eValid=1;
				}
			}
			/* skip trailing spaces */if(nDigits>0&&eValid>0) {
				while(zDx<length&&CharExtensions.sqlite3Isspace(z[zDx]))
					zDx+=incr;
			}
			do_atof_calc:
			/* adjust exponent by d, and update sign */e=(e*esign)+d;
			if(e<0) {
				esign=-1;
				e*=-1;
			}
			...
			/* if 0 significand */if(0==s) {
				/* In the IEEE 754 standard, zero is signed.
        ** Add the sign if we've seen at least one digit */result=(sign<0&&nDigits!=0)?-(double)0:(double)0;
			}
			else {
				/* attempt to reduce exponent */if(esign>0) {
				...
				/* adjust the sign of significand */s=sign<0?-s:s;
				/* if exponent, scale significand as appropriate
        ** and store in result. */if(e!=0) {
					double scale=1.0;
					/* attempt to handle extremely small/large numbers better */if(e>307&&e<342) {
						while((e%308)!=0) {
							scale*=1.0e+1;
							e-=1;
						}
            ...
            ** represented exactly. */while((e%22)!=0) {
							scale*=1.0e+1;
							e-=1;
						}
						while(e>0) {
		...
		public static int sqlite3Atoi64(string zNum,ref i64 pNum,int length,SqliteEncoding enc) {
			...
			/* Skip leading zeros. */for(i=zDx;i<length&&(c=zNum[i])>='0'&&c<='9';i+=incr) {
				u=u*10+(u64)(c-'0');
			}
        ...
        ** than 19 digits (thus guaranteeing that it is too large) */return 1;
			}
			else
				if(i-zDx<19*incr) {
					/* Less than 19 digits, so we know that it fits in 64 bits */Debug.Assert(u<=IntegerExtensions.LARGEST_INT64);
					return 0;
				}
				else {
					/* zNum is a 19-digit numbers.  Compare it against 9223372036854775808. */c=Sqlite3.compare2pow63(zNum.Substring(zDx),incr);
					if(c<0) {
						/* zNum is less than 9223372036854775808 so it fits */Debug.Assert(u<=IntegerExtensions.LARGEST_INT64);
						return 0;
					}
					else
						if(c>0) {
							/* zNum is greater than 9223372036854775808 so it overflows */return 1;
						}
						else {
							/* zNum is exactly 9223372036854775808.  Fits if negative.  The
          ** special case 2 overflow if positive */Debug.Assert(u-1==IntegerExtensions.LARGEST_INT64);
