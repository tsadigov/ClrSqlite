...
int sqlite3_blob_open(
  ...
  **
  ** After seeking the cursor, the vdbe executes an OpCode.OP_ResultRow.
  ...
  *ppBlob = 0;
  ...
  }else if( rc==SqlResult.SQLITE_OK ){
    sqlite3DbFree(db, zErr);
...
blob_open_out:
  if( v && (rc!=SqlResult.SQLITE_OK || db->mallocFailed) ){
...
static int blobReadWrite(
  ...
  if( n<0 || iOffset<0 || (iOffset+n)>p->nByte ){
    ...
    rc = SqlResult.SQLITE_ERROR;
    utilc.sqlite3Error(db, SqlResult.SQLITE_ERROR, 0);
  } else if( v==0 ){
