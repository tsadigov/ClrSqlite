...
*/public class sqlite3_backup {
			...
			/* Destination database handle */public Btree pDest;
			/* Destination b-tree file */public u32 iDestSchema;
			/* Original schema cookie in destination */public int bDestLocked;
			/* True once a write-transaction is open on pDest */public Pgno iNext;
			/* Page number of the next source page to copy */public sqlite3 pSrcDb;
			/* Source database handle */public Btree pSrc;
			/* Source b-tree file */public int rc;
			/* Backup process error code *//* These two variables are set by every call to backup_step(). They are
  ** read by calls to backup_remaining() and backup_pagecount().
  */public Pgno nRemaining;
			/* Number of pages left to copy */public Pgno nPagecount;
			/* Total number of pages to copy */public int isAttached;
			/* True once backup has been registered with pager */public sqlite3_backup pNext;
		...
		///</summary>
		static public sqlite3_backup sqlite3_backup_init(sqlite3 pDestDb,/* Database to write to */string zDestDb,/* Name of database within pDestDb */sqlite3 pSrcDb,/* Database connection to read from */string zSrcDb/* Name of database within pSrcDb */) {
			sqlite3_backup p;
			/* Value to return *//* Lock the source database handle. The destination database
  ...
  */sqlite3_mutex_enter(pSrcDb.mutex);
    ...
    ** sqlite3_backup_finish(). */p=new sqlite3_backup();
			...
			/* If the allocation succeeded, populate the new object. */if(p!=null) {
  ...
  */for(iOff=iEnd-(i64)nSrcPgsz;rc==SQLITE_OK&&iOff<iEnd;iOff+=nDestPgsz) {
      ...
      */Buffer.BlockCopy(zSrcData,(int)(iOff%nSrcPgsz),zDestData,(int)(iOff%nDestPgsz),nCopy);
		...
		static public int sqlite3_backup_step(sqlite3_backup p,int nPage) {
			int rc;
			int destMode;
			/* Destination journal mode */int pgszSrc=0;
			/* Source page size */int pgszDest=0;
			/* Destination page size */sqlite3_mutex_enter(p.pSrcDb.mutex);
			...
			if(!isFatalError(rc)) {
				...
				/* Source pager */Pager pDestPager=sqlite3BtreePager(p.pDest);
				/* Dest pager */int ii;
				/* Iterator variable */Pgno nSrcPage=0;
				/* Size of source db in pages */int bCloseTrans=0;
				/* True if src db requires unlocking *//* If the source pager is currently in a write-transaction, return
    ...
    */if(p.pDestDb!=null&&p.pSrc.pBt.inTransaction==TRANS_WRITE) {
				...
				/* Lock the destination database, if it is not locked already. */if(SQLITE_OK==rc&&p.bDestLocked==0&&SQLITE_OK==(rc=sqlite3BtreeBeginTrans(p.pDest,2))) {
					p.bDestLocked=1;
    ...
    ** and the page sizes are different between source and destination */pgszSrc=sqlite3BtreeGetPageSize(p.pSrc);
    ...
    */nSrcPage=sqlite3BtreeLastPage(p.pSrc);
				...
				for(ii=0;(nPage<0||ii<nPage)&&p.iNext<=nSrcPage&&0==rc;ii++) {
					Pgno iSrcPg=p.iNext;
					/* Source page number */if(iSrcPg!=PENDING_BYTE_PAGE(p.pSrc.pBt)) {
						DbPage pSrcPg=null;
    ...
    ** same schema version.
    */if(rc==SQLITE_DONE&&(rc=sqlite3BtreeUpdateMeta(p.pDest,1,p.iDestSchema+1))==SQLITE_OK) {
					Pgno nDestTruncate;
					if(p.pDestDb!=null) {
      ...
      */Debug.Assert(pgszSrc==sqlite3BtreeGetPageSize(p.pSrc));
        ...
        */int iSize=(int)(pgszSrc*nSrcPage);
						...
						Debug.Assert((i64)nDestTruncate*(i64)pgszDest>=iSize||(nDestTruncate==(int)(PENDING_BYTE_PAGE(p.pDest.pBt)-1)&&iSize>=PENDING_BYTE&&iSize<=PENDING_BYTE+pgszDest));
        ...
        ** journal file.  */rc=sqlite3PagerCommitPhaseOne(pDestPager,null,true);
					...
					/* Finish committing the transaction to the destination database. */if(SQLITE_OK==rc&&SQLITE_OK==(rc=sqlite3BtreeCommitPhaseTwo(p.pDest,0))) {
						rc=SQLITE_DONE;
					}
				}
    ...
    */if(bCloseTrans!=0) {
      ...
      Debug.Assert( rc2 == SQLITE_OK );
		...
		static public int sqlite3_backup_finish(sqlite3_backup p) {
			sqlite3_backup pp;
			/* Ptr to head of pagers backup list */sqlite3_mutex mutex;
			/* Mutex to protect source database */int rc;
			/* Value to return *//* Enter the mutexes */if(p==null)
				return SQLITE_OK;
			...
			/* Detach this backup from the source pager. */if(p.pDestDb!=null) {
				p.pSrc.nBackup--;
			}
			if(p.isAttached!=0) {
				pp=sqlite3PagerBackupPtr(sqlite3BtreePager(p.pSrc));
			...
			/* If a transaction is still open on the Btree, roll it back. */sqlite3BtreeRollback(p.pDest);
			/* Set the error code of the destination database handle. */rc=(p.rc==SQLITE_DONE)?SQLITE_OK:p.rc;
			sqlite3Error(p.pDestDb,rc,0);
			/* Exit the mutexes and free the backup context structure. */if(p.pDestDb!=null) {
				sqlite3_mutex_leave(p.pDestDb.mutex);
			}
			sqlite3BtreeLeave(p.pSrc);
			if(p.pDestDb!=null) {
				/* EVIDENCE-OF: R-64852-21591 The sqlite3_backup object is created by a
    ** call to sqlite3_backup_init() and is destroyed by a call to
    ** sqlite3_backup_finish(). *///sqlite3_free( ref p );
			}
		...
		static void sqlite3BackupUpdate(sqlite3_backup pBackup,Pgno iPage,byte[] aData) {
			sqlite3_backup p;
			/* Iterator variable */for(p=pBackup;p!=null;p=p.pNext) {
