    ...
    *///#define EXPR_FULLSIZE           sizeof(Expr)           /* Full size */
		...
		public class ExprSpan {
			...
			/* One character past the end of input text */public void spanSet(Token pStart,Token pEnd) {
				this.zStart=pStart.z;
				this.zEnd=pEnd.z.Substring(pEnd.n);
			}
			public void spanExpr(Parse pParse,int op,Token pValue) {
				this.pExpr=pParse.sqlite3PExpr(op,0,0,pValue);
				this.zStart=pValue.z;
				this.zEnd=pValue.z.Substring(pValue.n);
			}
			...
			public void spanUnaryPostfix(/* Write the new expression node here */Parse pParse,/* Parsing context to record errors */int op,/* The operator */ExprSpan pOperand,/* The operand */Token pPostOp/* The operand token for setting the span */) {
				...
				this.zEnd=pPostOp.z.Substring(pPostOp.n);
			}
      ...
      ** trigger sub-program.  */if((this.db.flags&SQLITE_ForeignKeys)!=0) {
			...
			public int sqlite3RunParser(string zSql,ref string pzErrMsg) {
				...
				while(/*  0 == db.mallocFailed && */i<zSql.Length) {
					...
					default: {
						pEngine.sqlite3Parser(tokenType,this.sLastToken,this);
...
*/return pWInfo;
      ...
      */if(!ExprHasAnyProperty(pExpr,EP_VarSelect)&&null==this.pTriggerTab) {
				...
				#if !SQLITE_OMIT_EXPLAIN
				if(this.explain==2) {
					string zMsg=sqlite3MPrintf(this.db,"EXECUTE %s%s SUBQUERY %d",testAddr!=0?"":"CORRELATED ",pExpr.op==TK_IN?"LIST":"SCALAR",this.iNextSelectId);
