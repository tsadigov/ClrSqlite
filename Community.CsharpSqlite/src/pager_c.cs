...
*///#define MAX_SECTOR_SIZE 0x10000
		...
		public class PagerSavepoint {
			public i64 iOffset;
			/* Starting offset in main journal */public i64 iHdrOffset;
			/* See above */public Bitvec pInSavepoint;
			/* Set of pages in this savepoint */public Pgno nOrig;
    ...
    */public class Pager {
			...
			/* OS functions to use for IO */public bool exclusiveMode;
			/* Boolean. True if locking_mode==EXCLUSIVE */public u8 journalMode;
			/* One of the PAGER_JOURNALMODE_* values */public u8 useJournal;
			/* Use a rollback journal on this file */public u8 noReadlock;
			/* Do not bother to obtain readlocks */public bool noSync;
			/* Do not sync the journal if true */public bool fullSync;
			/* Do extra syncs of the journal for robustness */public u8 ckptSyncFlags;
			/* SYNC_NORMAL or SYNC_FULL for checkpoint */public u8 syncFlags;
			/* SYNC_NORMAL or SYNC_FULL otherwise */public bool tempFile;
			/* zFilename is a temporary file */public bool readOnly;
			/* True for a read-only database */public bool alwaysRollback;
			/* Disable DontRollback() for all pages */public u8 memDb;
      ...
      */public u8 eState;
			/* Pager state (OPEN, READER, WRITER_LOCKED..) */public u8 eLock;
			/* Current lock held on database file */public bool changeCountDone;
			/* Set after incrementing the change-counter */public int setMaster;
			/* True if a m-j name has been written to jrnl */public u8 doNotSpill;
			/* Do not spill the cache when non-zero */public u8 doNotSyncSpill;
			/* Do not do a spill that requires jrnl sync */public u8 subjInMemory;
			/* True to use in-memory sub-journals */public Pgno dbSize;
			/* Number of pages in the database */public Pgno dbOrigSize;
			/* dbSize before the current transaction */public Pgno dbFileSize;
			/* Number of pages in the database file */public Pgno dbHintSize;
			/* Value passed to FCNTL_SIZE_HINT call */public int errCode;
			/* One of several kinds of errors */public int nRec;
			/* Pages journalled since last j-header written */public u32 cksumInit;
			/* Quasi-random value added to every checksum */public u32 nSubRec;
			/* Number of records written to sub-journal */public Bitvec pInJournal;
			/* One bit for each page in the database file */public sqlite3_file fd;
			/* File descriptor for database */public sqlite3_file jfd;
      ...
      ***************************************************************************/public u16 nExtra;
			/* Add this many bytes to each in-memory page */public i16 nReserve;
			/* Number of unused bytes at end of each page */public u32 vfsFlags;
			/* Flags for sqlite3_vfs.xOpen() */public u32 sectorSize;
			/* Assumed sector size during rollback */public int pageSize;
			/* Number of bytes in a page */public Pgno mxPgno;
			/* Maximum allowed size of the database */public i64 journalSizeLimit;
			/* Size limit for persistent journal files */public string zFilename;
			/* Name of the database file */public string zJournal;
			/* Name of the journal file */public dxBusyHandler xBusyHandler;
      ...
      public int nRead, nWrite;            /* Database pages read/written */
			...
			/* Routine for en/decoding data *///void (*xCodecSizeChng)(void*,int,int); 
			public dxCodecSizeChng xCodecSizeChng;
			/* Notify of page size changes *///void (*xCodecFree)(void*);             
			public dxCodecFree xCodecFree;
			/* Destructor for the codec */public codec_ctx pCodec;
...
*/static bool assert_pager_state(Pager p) {
			Pager pPager=p;
			/* State must be valid. */Debug.Assert(p.eState==PAGER_OPEN||p.eState==PAGER_READER||p.eState==PAGER_WRITER_LOCKED||p.eState==PAGER_WRITER_CACHEMOD||p.eState==PAGER_WRITER_DBMOD||p.eState==PAGER_WRITER_FINISHED||p.eState==PAGER_ERROR);
      ...
      */if(
			...
			0!=pPager.memDb
				...
				Debug.Assert(p.journalMode==PAGER_JOURNALMODE_OFF||p.journalMode==PAGER_JOURNALMODE_MEMORY);
      ...
      */Debug.Assert(pPager.changeCountDone==false||pPager.eLock>=RESERVED_LOCK);
            ...
            ** to journal_mode=wal.
            */Debug.Assert(p.eLock>=RESERVED_LOCK);
				Debug.Assert(isOpen(p.jfd)||p.journalMode==PAGER_JOURNALMODE_OFF||p.journalMode==PAGER_JOURNALMODE_WAL);
			}
			Debug.Assert(pPager.dbOrigSize==pPager.dbFileSize);
			Debug.Assert(pPager.dbOrigSize==pPager.dbHintSize);
			break;
...
assert( (pPg->flags&PGHDR_DIRTY) || pPg->pageHash==pager_pagehash(pPg) );
		...
		static int readMasterJournal(sqlite3_file pJrnl,byte[] zMaster,u32 nMaster) {
			int rc;
			/* Return code */int len=0;
			/* Length in bytes of master journal name */i64 szJ=0;
			/* Total size in bytes of journal file pJrnl */u32 cksum=0;
			/* MJ checksum value read from journal */int u;
			/* Unsigned loop counter */byte[] aMagic=new byte[8];
			/* A buffer to hold the magic header */zMaster[0]=0;
			...
			/* See if the checksum matches the master journal name */for(u=0;u<len;u++) {
				cksum-=zMaster[u];
			}
			if(cksum!=0) {
        ...
        */len=0;
		...
		static int zeroJournalHdr(Pager pPager,int doTruncate) {
			int rc=SQLITE_OK;
			/* Return code */Debug.Assert(isOpen(pPager.jfd));
			if(pPager.journalOff!=0) {
				i64 iLimit=pPager.journalSizeLimit;
				/* Local cache of jsl */IOTRACE("JZEROHDR %p\n",pPager);
        ...
        */if(rc==SQLITE_OK&&iLimit>0) {
		...
		static int writeJournalHdr(Pager pPager) {
			int rc=SQLITE_OK;
			/* Return code */byte[] zHeader=pPager.pTmpSpace;
			/* Temporary space used to build header */u32 nHeader=(u32)pPager.pageSize;
			/* Size of buffer pointed to by zHeader */u32 nWrite;
			/* Bytes of header sector written */int ii;
			/* Loop counter */Debug.Assert(isOpen(pPager.jfd));
			/* Journal file must be open. */if(nHeader>JOURNAL_HDR_SZ(pPager)) {
      ...
      */Debug.Assert(isOpen(pPager.fd)||pPager.noSync);
			if(pPager.noSync||(pPager.journalMode==PAGER_JOURNALMODE_MEMORY)||(sqlite3OsDeviceCharacteristics(pPager.fd)&SQLITE_IOCAP_SAFE_APPEND)!=0) {
			...
			/* The random check-hash initialiser */i64 i64Temp=0;
			...
			/* The initial database size */put32bits(zHeader,aJournalMagic.Length+8,pPager.dbOrigSize);
			/* The assumed sector size for this process */put32bits(zHeader,aJournalMagic.Length+12,pPager.sectorSize);
			/* The page size */put32bits(zHeader,aJournalMagic.Length+16,(u32)pPager.pageSize);
      ...
      */for(nWrite=0;rc==SQLITE_OK&&nWrite<JOURNAL_HDR_SZ(pPager);nWrite+=nHeader) {
		...
		///
		///</summary>
		static int readJournalHdr(Pager pPager,/* Pager object */int isHot,i64 journalSize,/* Size of the open journal file in bytes */out u32 pNRec,/* OUT: Value read from the nRec field */out u32 pDbSize/* OUT: Value of original database size field */) {
			int rc;
			/* Return code */byte[] aMagic=new byte[8];
			/* A buffer to hold the magic header */i64 iHdrOff;
			/* Offset of journal header being read */Debug.Assert(isOpen(pPager.jfd));
			/* Journal file must be open. */pNRec=0;
			pDbSize=0;
      ...
      */if(SQLITE_OK!=(rc=read32bits(pPager.jfd,iHdrOff+8,ref pNRec))||SQLITE_OK!=(rc=read32bits(pPager.jfd,iHdrOff+12,ref pPager.cksumInit))||SQLITE_OK!=(rc=read32bits(pPager.jfd,iHdrOff+16,ref pDbSize))) {
				return rc;
			}
			if(pPager.journalOff==0) {
				u32 iPageSize=0;
				/* Page-size field of journal header */u32 iSectorSize=0;
				/* Sector-size field of journal header *//* Read the page-size and sector-size journal header fields. */if(SQLITE_OK!=(rc=read32bits(pPager.jfd,iHdrOff+20,ref iSectorSize))||SQLITE_OK!=(rc=read32bits(pPager.jfd,iHdrOff+24,ref iPageSize))) {
					return rc;
				}
        ...
        */pPager.sectorSize=iSectorSize;
		...
		static int writeMasterJournal(Pager pPager,string zMaster) {
			int rc;
			/* Return code */int nMaster;
			/* Length of string zMaster */i64 iHdrOff;
			/* Offset of header in journal file */i64 jrnlSize=0;
			/* Size of journal file on disk */u32 cksum=0;
			/* Checksum of string zMaster */Debug.Assert(pPager.setMaster==0);
			Debug.Assert(!pagerUseWal(pPager));
			if(null==zMaster||pPager.journalMode==PAGER_JOURNALMODE_MEMORY||pPager.journalMode==PAGER_JOURNALMODE_OFF) {
				return SQLITE_OK;
			}
			pPager.setMaster=1;
			Debug.Assert(isOpen(pPager.jfd));
			Debug.Assert(pPager.journalHdr<=pPager.journalOff);
			/* Calculate the length in bytes and the checksum of zMaster */for(nMaster=0;nMaster<zMaster.Length&&zMaster[nMaster]!=0;nMaster++) {
      ...
      */if(SQLITE_OK==(rc=sqlite3OsFileSize(pPager.jfd,ref jrnlSize))&&jrnlSize>pPager.journalOff) {
		...
		static PgHdr pager_lookup(Pager pPager,u32 pgno) {
			PgHdr p=null;
			/* Return value *//* It is not possible for a call to PcacheFetch() with createFlag==0 to
      ...
      */sqlite3PcacheFetch(pPager.pPCache,pgno,0,ref p);
		...
		static void releaseAllSavepoints(Pager pPager) {
			int ii;
			/* Iterator for looping through Pager.aSavepoint */for(ii=0;ii<pPager.nSavepoint;ii++) {
		...
		static int addToSavepointBitvecs(Pager pPager,u32 pgno) {
			int ii;
			/* Loop counter */int rc=SQLITE_OK;
			/* Result code */for(ii=0;ii<pPager.nSavepoint;ii++) {
		...
		static void pager_unlock(Pager pPager) {
			Debug.Assert(pPager.eState==PAGER_READER||pPager.eState==PAGER_OPEN||pPager.eState==PAGER_ERROR);
			...
			if(pagerUseWal(pPager)) {
				...
				if(!pPager.exclusiveMode) {
					int rc;
					/* Error code returned by pagerUnlockDb() */int iDc=isOpen(pPager.fd)?sqlite3OsDeviceCharacteristics(pPager.fd):0;
      ...
      */if(pPager.errCode!=0) {
		...
		static int pager_error(Pager pPager,int rc) {
			...
			Debug.Assert(pPager.errCode==SQLITE_FULL||pPager.errCode==SQLITE_OK||(pPager.errCode&0xff)==SQLITE_IOERR);
		...
		static int pager_end_transaction(Pager pPager,int hasMaster) {
			int rc=SQLITE_OK;
			/* Error code from journal finalization operation */int rc2=SQLITE_OK;
			/* Error code from db file unlock operation *//* Do nothing if the pager does not have an open write transaction
      ...
      */Debug.Assert(assert_pager_state(pPager));
			...
			if(isOpen(pPager.jfd)) {
				Debug.Assert(!pagerUseWal(pPager));
				/* Finalize the journal file. */if(sqlite3IsMemJournal(pPager.jfd)) {
					...
					if(pPager.journalMode==PAGER_JOURNALMODE_TRUNCATE) {
						...
						if(pPager.journalMode==PAGER_JOURNALMODE_PERSIST||(pPager.exclusiveMode&&pPager.journalMode!=PAGER_JOURNALMODE_WAL)) {
          ...
          */Debug.Assert(pPager.journalMode==PAGER_JOURNALMODE_DELETE||pPager.journalMode==PAGER_JOURNALMODE_MEMORY||pPager.journalMode==PAGER_JOURNALMODE_WAL);
							sqlite3OsClose(pPager.jfd);
							if(!pPager.tempFile) {
...
sqlite3PagerUnref(p);
    ...
    */static u32 pager_cksum(Pager pPager,byte[] aData) {
			...
			/* Checksum value to return */int i=pPager.pageSize-200;
			/* Loop counter */while(i>0) {
				cksum+=aData[i];
				i-=200;
			}
			return cksum;
...
static void pagerReportSize(Pager X){}
		...
		///</summary>
		static int pager_playback_one_page(Pager pPager,/* The pager being played back */ref i64 pOffset,/* Offset of record to playback */Bitvec pDone,/* Bitvec of pages already played back */int isMainJrnl,/* True for main rollback journal. False for Stmt jrnl */int isSavepnt/* True for a savepoint rollback */) {
			int rc;
			PgHdr pPg;
			/* An existing page in the cache */Pgno pgno=0;
			/* The page number of a page in journal */u32 cksum=0;
			/* Checksum used for sanity checking */byte[] aData;
			/* Temporary storage for the page */sqlite3_file jfd;
			/* The file descriptor for the journal file */bool isSynced;
			/* True if journal page is synced */Debug.Assert((isMainJrnl&~1)==0);
			/* isMainJrnl is 0 or 1 */Debug.Assert((isSavepnt&~1)==0);
			/* isSavepnt is 0 or 1 */Debug.Assert(isMainJrnl!=0||pDone!=null);
			/* pDone always used on sub-journals */Debug.Assert(isSavepnt!=0||pDone==null);
			/* pDone never used on non-savepoint */aData=pPager.pTmpSpace;
			Debug.Assert(aData!=null);
			/* Temp storage must have already been allocated */Debug.Assert(pagerUseWal(pPager)==false||(0==isMainJrnl&&isSavepnt!=0));
			/* Either the state is greater than PAGER_WRITER_CACHEMOD (a transaction 
      ** or savepoint rollback done at the request of the caller) or this is
      ** a hot-journal rollback. If it is a hot-journal rollback, the pager
      ** is in state OPEN and holds an EXCLUSIVE lock. Hot-journal rollback
      ** only reads from the main journal, not the sub-journal.
      */Debug.Assert(pPager.eState>=PAGER_WRITER_CACHEMOD||(pPager.eState==PAGER_OPEN&&pPager.eLock==EXCLUSIVE_LOCK));
      ...
      */if(pagerUseWal(pPager)) {
			...
			PAGERTRACE("PLAYBACK %d page %d hash(%08x) %s\n",PAGERID(pPager),pgno,pager_datahash(pPager.pageSize,aData),(isMainJrnl!=0?"main-journal":"sub-journal"));
			if(isMainJrnl!=0) {
			...
			if(isOpen(pPager.fd)&&(pPager.eState>=PAGER_WRITER_DBMOD||pPager.eState==PAGER_OPEN)&&isSynced) {
				...
				if(pPager.pBackup!=null) {
					...
					pPager.pBackup.sqlite3BackupUpdate(pgno,(u8[])aData);
        ...
        ** Do this before any decoding. */if(pgno==1) {
					Buffer.BlockCopy(pData,24,pPager.dbFileVers,0,pPager.dbFileVers.Length);
					//memcpy(pPager.dbFileVers, ((u8*)pData)[24], sizeof(pPager.dbFileVers));
				}
				/* Decode the page just read from disk */if(CODEC1(pPager,pData,pPg.pgno,SQLITE_DECRYPT))
					rc=SQLITE_NOMEM;
		...
		static int pager_delmaster(Pager pPager,string zMaster) {
			...
			int rc;
			/* Return code */sqlite3_file pMaster;
			/* Malloc'd master-journal file descriptor */sqlite3_file pJournal;
			/* Malloc'd child-journal file descriptor *///string zMasterJournal = null; /* Contents of master journal file */
			i64 nMasterJournal;
			/* Size of master journal file */string zJournal;
			/* Pointer to one journal within MJ file */string zMasterPtr;
			/* Space to hold MJ filename from a journal file */int nMasterPtr;
			/* Amount of space allocated to zMasterPtr[] *//* Allocate space for both the pJournal and pMaster file descriptors.
      ...
      *///rc = sqlite3OsFileSize(pMaster, &nMasterJournal);
		...
		static int pager_truncate(Pager pPager,u32 nPage) {
			...
			Debug.Assert(pPager.eState!=PAGER_ERROR);
			Debug.Assert(pPager.eState!=PAGER_READER);
			if(isOpen(pPager.fd)&&(pPager.eState>=PAGER_WRITER_DBMOD||pPager.eState==PAGER_OPEN)) {
				i64 currentSize=0,newSize;
				int szPage=pPager.pageSize;
				Debug.Assert(pPager.eLock==EXCLUSIVE_LOCK);
				/* TODO: Is it safe to use Pager.dbFileSize here? */rc=sqlite3OsFileSize(pPager.fd,ref currentSize);
        ...
        */pPager.sectorSize=(u32)sqlite3OsSectorSize(pPager.fd);
		...
		static int pager_playback(Pager pPager,int isHot) {
			sqlite3_vfs pVfs=pPager.pVfs;
			i64 szJ=0;
			/* Size of the journal file in bytes */u32 nRec=0;
			/* Number of Records in the journal */u32 u;
			/* Unsigned loop counter */u32 mxPg=0;
			/* Size of the original file in pages */int rc;
			/* Result code of a subroutine */int res=1;
			/* Value returned by sqlite3OsAccess() */byte[] zMaster=null;
			/* Name of master journal file if any */int needPagerReset;
			/* True to reset page prior to first page rollback *//* Figure out how many records are in the journal.  Abort early if
      ...
      */while(true) {
        ...
        */for(u=0;u<nRec;u++) {
              ...
              ** first place so it is OK to simply abandon the rollback. */rc=SQLITE_OK;
								goto end_playback;
							}
							else {
			...
			/*NOTREACHED*/end_playback:
			/* Following a rollback, the database file should be back in its original
      ** state prior to the start of the transaction, so invoke the
      ** SQLITE_FCNTL_DB_UNCHANGED file-control method to disable the
      ** assertion that the transaction counter was modified.
      */sqlite3_int64 iDummy=0;
			Debug.Assert(pPager.fd.pMethods==null||sqlite3OsFileControl(pPager.fd,SQLITE_FCNTL_DB_UNCHANGED,ref iDummy)>=SQLITE_OK);
      ...
      */setSectorSize(pPager);
		...
		static int readDbPage(PgHdr pPg) {
			Pager pPager=pPg.pPager;
			/* Pager object associated with page pPg */Pgno pgno=pPg.pgno;
			/* Page number to read */int rc=SQLITE_OK;
			/* Return code */int isInWal=0;
			...
			if(pagerUseWal(pPager)) {
				/* Try to pull the page from the write-ahead log. */rc=sqlite3WalRead(pPager.pWal,pgno,ref isInWal,pgsz,pPg.pData);
			}
      ...
      PAGER_INCR( ref pPager.nRead );
		...
		static void pager_write_changecounter(PgHdr pPg) {
			u32 change_counter;
			/* Increment the value just read and write it back to byte 24. */change_counter=Converter.sqlite3Get4byte(pPg.pPager.dbFileVers,0)+1;
			put32bits(pPg.pData,24,change_counter);
			/* Also store the SQLite version number in bytes 96..99 and in
      ** bytes 92..95 store the change counter for which the version number
      ** is valid. */put32bits(pPg.pData,92,change_counter);
...
return rc;
		...
		static int pagerPagecount(Pager pPager,ref Pgno pnPage) {
			Pgno nPage;
			/* Value to return via *pnPage *//* Query the WAL sub-system for the database size. The WalDbsize()
      ...
      */if(nPage==0) {
				i64 n=0;
				/* Size of db file in bytes */Debug.Assert(isOpen(pPager.fd)||pPager.tempFile);
				if(isOpen(pPager.fd)) {
...
return rc;
		...
		static int pagerPlaybackSavepoint(Pager pPager,PagerSavepoint pSavepoint) {
			i64 szJ;
			/* Effective size of the main journal */i64 iHdrOff;
			/* End of first segment of main-journal records */int rc=SQLITE_OK;
			/* Return code */Bitvec pDone=null;
			/* Bitvec to ensure pages played back only once */Debug.Assert(pPager.eState!=PAGER_ERROR);
			Debug.Assert(pPager.eState>=PAGER_WRITER_LOCKED);
			/* Allocate a bitvec to use to store the set of pages rolled back */if(pSavepoint!=null) {
      ...
      */while(rc==SQLITE_OK&&pPager.journalOff<szJ) {
				u32 ii;
				/* Loop counter */u32 nJRec;
				/* Number of Journal Records */u32 dummy;
      ...
      */if(pSavepoint!=null) {
				u32 ii;
				/* Loop counter */i64 offset=pSavepoint.iSubRec*(4+pPager.pageSize);
				if(pagerUseWal(pPager)) {
    ...
    */static void sqlite3PagerSetCachesize(Pager pPager,int mxPage) {
		...
		static void sqlite3PagerSetSafetyLevel(Pager pPager,/* The pager to set safety level for */int level,/* PRAGMA synchronous.  1=OFF, 2=NORMAL, 3=FULL */int bFullFsync,/* PRAGMA fullfsync */int bCkptFullFsync/* PRAGMA checkpoint_fullfsync */) {
		...
		///</summary>
		static int pagerOpentemp(Pager pPager,/* The pager object */ref sqlite3_file pFile,/* Write the file descriptor here */int vfsFlags/* Flags passed through to the VFS */) {
			int rc;
			/* Return code */
			#if SQLITE_TEST
												#if !TCLSH
		...
		///
		///</summary>
		static void sqlite3PagerSetBusyhandler(Pager pPager,/* Pager object */dxBusyHandler xBusyHandler,/* Pointer to busy-handler function *///int (*xBusyHandler)(void *),
      ...
      */u32 pageSize=pPageSize;
			...
			if((pPager.memDb==0||pPager.dbSize==0)&&sqlite3PcacheRefCount(pPager.pPCache)==0&&pageSize!=0&&pageSize!=(u32)pPager.pageSize) {
				...
				if(rc==SQLITE_OK) {
					...
					pPager.pTmpSpace=sqlite3Malloc(pageSize);
					// pNew;
    ...
    */static Pgno sqlite3PagerMaxPageCount(Pager pPager,int mxPage) {
			...
			Debug.Assert(pPager.eState!=PAGER_OPEN);
			/* Called only by OP_MaxPgcnt */Debug.Assert(pPager.mxPgno>=pPager.dbSize);
			/* OP_MaxPgcnt enforces this */return pPager.mxPgno;
		}
		///<summary>
    ...
    */static int pager_wait_on_lock(Pager pPager,int locktype) {
			int rc;
			/* Return code *//* Check that this is either a no-op (because the requested lock is
      ** already held, or one of the transistions that the busy-handler
      ** may be invoked during, according to the comment above
      ** sqlite3PagerSetBusyhandler().
      */Debug.Assert((pPager.eLock>=locktype)||(pPager.eLock==NO_LOCK&&locktype==SHARED_LOCK)||(pPager.eLock==RESERVED_LOCK&&locktype==EXCLUSIVE_LOCK));
			do {
...
#endif
			sqlite3BeginBenignMalloc();
			/* pPager.errCode = 0; */pPager.exclusiveMode=false;
			#if !SQLITE_OMIT_WAL
    ...
    static Pgno sqlite3PagerPagenumber( DbPage pPg )
    {
      return pPg.pgno;
		...
		static int syncJournal(Pager pPager,int newHdr) {
			int rc=SQLITE_OK;
			Debug.Assert(pPager.eState==PAGER_WRITER_CACHEMOD||pPager.eState==PAGER_WRITER_DBMOD);
			Debug.Assert(assert_pager_state(pPager));
			Debug.Assert(!pagerUseWal(pPager));
      ...
      */sqlite3PcacheClearSyncFlags(pPager.pPCache);
		...
		static int pager_write_pagelist(Pager pPager,PgHdr pList) {
			int rc=SQLITE_OK;
			/* Return code *//* This function is only called for rollback pagers in WRITER_DBMOD state. */Debug.Assert(!pagerUseWal(pPager));
      ...
      */Debug.Assert(rc!=SQLITE_OK||isOpen(pPager.fd));
        ...
        */if(pList.pgno<=pPager.dbSize&&0==(pList.flags&PGHDR_DONT_WRITE)) {
					...
					/* Offset to write */byte[] pData=null;
					/* Data to write */Debug.Assert((pList.flags&PGHDR_NEED_SYNC)==0);
					if(pList.pgno==1)
						pager_write_changecounter(pList);
					/* Encode the database */if(CODEC2(pPager,pList.pData,pgno,SQLITE_ENCRYPT_WRITE_CTX,ref pData))
						return SQLITE_NOMEM;
					//     CODEC2(pPager, pList.pData, pgno, 6, return SQLITE_NOMEM, pData);
					/* Write out the page data. */rc=sqlite3OsWrite(pPager.fd,pData,pPager.pageSize,offset);
          ...
          */if(pgno==1) {
					...
					/* Update any backup objects copying the contents of this pager. */pPager.pBackup.sqlite3BackupUpdate(pgno,pList.pData);
		...
		static int subjournalPage(PgHdr pPg) {
			...
			if(pPager.journalMode!=PAGER_JOURNALMODE_OFF) {
				/* Open the sub-journal, if it has not already been opened */Debug.Assert(pPager.useJournal!=0);
        ...
        ** write the journal record into the file.  */if(rc==SQLITE_OK) {
					byte[] pData=pPg.pData;
      ...
      */if(NEVER(pPager.errCode!=0))
			...
			pPg.pDirty=null;
			if(pagerUseWal(pPager)) {
				/* Write a single frame for this page to the log. */if(subjRequiresPage(pPg)) {
				...
				/* Sync the journal file if required. */if((pPg.flags&PGHDR_NEED_SYNC)!=0||pPager.eState==PAGER_WRITER_CACHEMOD) {
        ...
        ** executed.
        */if(NEVER(rc==SQLITE_OK&&pPg.pgno>pPager.dbSize&&subjRequiresPage(pPg))) {
					rc=subjournalPage(pPg);
				}
				/* Write the contents of the page out to the database file. */if(rc==SQLITE_OK) {
					Debug.Assert((pPg.flags&PGHDR_NEED_SYNC)==0);
					rc=pager_write_pagelist(pPager,pPg);
				}
			}
			/* Mark the page as clean. */if(rc==SQLITE_OK) {
		...
		///
		///</summary>
		static int sqlite3PagerOpen(sqlite3_vfs pVfs,/* The virtual file system to use */out Pager ppPager,/* OUT: Return the Pager structure here */string zFilename,/* Name of the database file to open */int nExtra,/* Extra bytes append to each in-memory page */int flags,/* flags controlling this file */int vfsFlags,/* flags passed through to sqlite3_vfs.xOpen() */dxReiniter xReinit/* Function to reinitialize pages */) {
			u8 pPtr;
			Pager pPager=null;
			/* Pager object to allocate and return */int rc=SQLITE_OK;
			/* Return code */u8 tempFile=0;
			/* True for temp files (incl. in-memory files) */// Needs to be u8 for later tests
			u8 memDb=0;
			/* True if this is an in-memory file */bool readOnly=false;
			/* True if this is a read-only file */int journalFileSize;
			/* Bytes to allocate for each journal fd */StringBuilder zPathname=null;
			/* Full path to database file */int nPathname=0;
			/* Number of bytes in zPathname */bool useJournal=(flags&PAGER_OMIT_JOURNAL)==0;
			/* False to omit journal */bool noReadlock=(flags&PAGER_NO_READLOCK)!=0;
			/* True to omit read-lock */int pcacheSize=sqlite3PcacheSize();
			/* Bytes to allocate for PCache */u32 szPageDflt=SQLITE_DEFAULT_PAGE_SIZE;
			/* Default page size */string zUri=null;
			/* URI args to copy */int nUri=0;
			/* Number of bytes of URI args at *zUri *//* Figure out how much space is required for each journal file-handle
      ...
      */if(sqlite3JournalSize(pVfs)>sqlite3MemJournalSize()) {
			...
			/* Set the output variable to NULL in case an error occurs. */ppPager=null;
			#if !SQLITE_OMIT_MEMORYDB
      ...
      *///pPtr = (u8 *)sqlite3MallocZero(
			...
			/* Fill in the Pager.zFilename and Pager.zJournal buffers, if required. */if(zPathname!=null) {
...
memcpy(&pPager.zWal[nPathname], "-wal", 4);
      ...
      */if(!String.IsNullOrEmpty(zFilename)) {
				int fout=0;
				/* VFS flags returned by xOpen() */rc=sqlite3OsOpen(pVfs,pPager.zFilename,pPager.fd,vfsFlags,ref fout);
				Debug.Assert(0==memDb);
...
szPageDflt = ii;
      ...
      */if(rc!=SQLITE_OK) {
			...
			/* Initialize the PCache object. */Debug.Assert(nExtra<1000);
			...
			/* pPager.pFirst = 0; *//* pPager.pFirstSynced = 0; *//* pPager.pLast = 0; */pPager.nExtra=(u16)nExtra;
			...
			/* pPager.xBusyHandler = 0; *//* pPager.pBusyHandlerArg = 0; */pPager.xReiniter=xReinit;
			/* memset(pPager.aHash, 0, sizeof(pPager.aHash)); */ppPager=pPager;
			return SQLITE_OK;
		}
		///<summary>
		...
		static int hasHotJournal(Pager pPager,ref int pExists) {
			sqlite3_vfs pVfs=pPager.pVfs;
			int rc=SQLITE_OK;
			/* Return code */int exists=1;
			/* True if a journal file is present */int jrnlOpen=isOpen(pPager.jfd)?1:0;
			...
			if(rc==SQLITE_OK&&exists!=0) {
				int locked=0;
				/* True if some process holds a RESERVED lock *//* Race condition here:  Another process might have been holding the
        ...
        */rc=sqlite3OsCheckReservedLock(pPager.fd,ref locked);
				if(rc==SQLITE_OK&&locked==0) {
					Pgno nPage=0;
					/* Number of pages in database file *//* Check the size of the database file. If it consists of 0 pages,
		...
		static int sqlite3PagerSharedLock(Pager pPager) {
			int rc=SQLITE_OK;
			/* Return code *//* This routine is only called from b-tree and only when there are no
      ...
      */Debug.Assert(sqlite3PcacheRefCount(pPager.pPCache)==0);
        ...
        */if(pPager.eLock<=SHARED_LOCK) {
          ...
          */if(!isOpen(pPager.jfd)) {
						sqlite3_vfs pVfs=pPager.pVfs;
						int bExists=0;
						/* True if journal file exists */rc=sqlite3OsAccess(pVfs,pPager.zJournal,SQLITE_ACCESS_EXISTS,ref bExists);
          ...
          */if(isOpen(pPager.jfd)) {
            ...
            */pager_error(pPager,rc);
					...
					Debug.Assert(pPager.eState==PAGER_OPEN);
					Debug.Assert((pPager.eLock==SHARED_LOCK)||(pPager.exclusiveMode&&pPager.eLock>SHARED_LOCK));
				}
    ...
    */static void pagerUnlockIfUnused(Pager pPager) {
		...
		static int sqlite3PagerGet(Pager pPager,/* The pager open on the database file */u32 pgno,/* Page number to fetch */ref DbPage ppPage/* Write a pointer to the page here */) {
			return sqlite3PagerAcquire(pPager,pgno,ref ppPage,0);
		}
		static int sqlite3PagerAcquire(Pager pPager,/* The pager open on the database file */u32 pgno,/* Page number to fetch */ref DbPage ppPage,/* Write a pointer to the page here */u8 noContent/* Do not bother reading content from disk if true */) {
			int rc;
			PgHdr pPg=null;
			Debug.Assert(pPager.eState>=PAGER_READER);
			Debug.Assert(assert_pager_state(pPager));
      ...
      ** Otherwise, request the page from the PCache layer. */if(pPager.errCode!=SQLITE_OK) {
				rc=pPager.errCode;
			}
			else {
        ...
        ** Set pPg to 0 and jump to the exception handler.  */pPg=null;
        ...
        ** the page. Return without further ado.  */Debug.Assert(pgno<=PAGER_MAX_PGNO&&pgno!=PAGER_MJ_PGNO(pPager));
        ...
        ** number greater than this, or the unused locking-page, is requested. */if(pgno>PAGER_MAX_PGNO||pgno==PAGER_MJ_PGNO(pPager)) {
		...
		static int pager_open_journal(Pager pPager) {
			int rc=SQLITE_OK;
			/* Return code */sqlite3_vfs pVfs=pPager.pVfs;
			/* Local cache of vfs pointer */Debug.Assert(pPager.eState==PAGER_WRITER_LOCKED);
			Debug.Assert(assert_pager_state(pPager));
			Debug.Assert(pPager.pInJournal==null);
			/* If already in the error state, this function is a no-op.  But on
      ** the other hand, this routine is never called if we are already in
      ** an error state. */if(NEVER(pPager.errCode)!=0)
				return pPager.errCode;
			if(!pagerUseWal(pPager)&&pPager.journalMode!=PAGER_JOURNALMODE_OFF) {
				...
				//  return SQLITE_NOMEM;
				//}
				/* Open the journal file if it is not already open. */if(!isOpen(pPager.jfd)) {
					if(pPager.journalMode==PAGER_JOURNALMODE_MEMORY) {
						sqlite3MemJournalOpen(pPager.jfd);
					}
					else {
						int flags=/* VFS flags to open journal file */SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE|(pPager.tempFile?(SQLITE_OPEN_DELETEONCLOSE|SQLITE_OPEN_TEMP_JOURNAL):(SQLITE_OPEN_MAIN_JOURNAL));
						#if SQLITE_ENABLE_ATOMIC_WRITE
...
pVfs, pPager.zJournal, pPager.jfd, flags, jrnlBufferSize(pPager)
        ...
        */if(rc==SQLITE_OK) {
					/* TODO: Check if all of these are really required. */pPager.nRec=0;
					pPager.journalOff=0;
					pPager.setMaster=0;
					pPager.journalHdr=0;
					rc=writeJournalHdr(pPager);
      ...
      ** again. This should not happen, but the check provides robustness. */if(NEVER(pPager.errCode)!=0)
				return pPager.errCode;
...
*/if(pPager.eState==PAGER_WRITER_LOCKED) {
      ...
      */sqlite3PcacheMakeDirty(pPg);
        ...
        */if(!pageInJournal(pPg)&&!pagerUseWal(pPager)) {
            ...
            ** that we do not. */Debug.Assert(pPg.pgno!=((PENDING_BYTE/(pPager.pageSize))+1));
...
#endif
																								#endif
						PAGERTRACE("JOURNAL %d page %d needSync=%d hash(%08x)\n",PAGERID(pPager),pPg.pgno,((pPg.flags&PGHDR_NEED_SYNC)!=0?1:0),pager_pagehash(pPg));
    ...
    */static int sqlite3PagerWrite(DbPage pDbPage) {
			...
			if(nPagePerSector>1) {
				Pgno nPageCount=0;
				/* Total number of pages in database file */Pgno pg1;
				/* First page of the sector pPg is located on. */Pgno nPage=0;
				/* Number of pages starting at pg1 to journal */int ii;
				/* Loop counter */bool needSync=false;
				/* True if any page has PGHDR_NEED_SYNC *//* Set the doNotSyncSpill flag to 1. This is because we cannot allow
        ...
        */pg1=(u32)((pPg.pgno-1)&~(nPagePerSector-1))+1;
				...
				for(ii=0;ii<nPage&&rc==SQLITE_OK;ii++) {
					...
					if(pg==pPg.pgno||sqlite3BitvecTest(pPager.pInJournal,pg)==0) {
						...
						if((pPage=pager_lookup(pPager,pg))!=null) {
							if((pPage.flags&PGHDR_NEED_SYNC)!=0) {
								needSync=true;
							}
							sqlite3PagerUnref(pPage);
...
int DIRECT_MODE = isDirectMode;
			...
			if(!pPager.changeCountDone&&pPager.dbSize>0) {
				PgHdr pPgHdr=null;
				/* Reference to page 1 */Debug.Assert(!pPager.tempFile&&isOpen(pPager.fd));
				/* Open page 1 of the file for writing. */rc=sqlite3PagerGet(pPager,1,ref pPgHdr);
        ...
        */if(!DIRECT_MODE&&ALWAYS(rc==SQLITE_OK)) {
				...
				if(rc==SQLITE_OK) {
					/* Actually do the update of the change counter */pager_write_changecounter(pPgHdr);
					/* If running in direct mode, write the contents of page 1 to the file. */if(DIRECT_MODE) {
						u8[] zBuf=null;
				...
				/* Release the page reference. */sqlite3PagerUnref(pPgHdr);
			}
			return rc;
		}
		///<summary>
		...
		static int sqlite3PagerExclusiveLock(Pager pPager) {
			int rc=SQLITE_OK;
			Debug.Assert(pPager.eState==PAGER_WRITER_CACHEMOD||pPager.eState==PAGER_WRITER_DBMOD||pPager.eState==PAGER_WRITER_LOCKED);
			Debug.Assert(assert_pager_state(pPager));
		...
		///
		///</summary>
		static int sqlite3PagerCommitPhaseOne(Pager pPager,/* Pager object */string zMaster,/* If not NULL, the master journal name */bool noSync/* True to omit the xSync on the db file */) {
			int rc=SQLITE_OK;
			/* Return code */Debug.Assert(pPager.eState==PAGER_WRITER_LOCKED||pPager.eState==PAGER_WRITER_CACHEMOD||pPager.eState==PAGER_WRITER_DBMOD||pPager.eState==PAGER_ERROR);
			Debug.Assert(assert_pager_state(pPager));
			/* If a prior error occurred, report that error again. */if(NEVER(pPager.errCode!=0))
				return pPager.errCode;
			PAGERTRACE("DATABASE SYNC: File=%s zMaster=%s nSize=%d\n",pPager.zFilename,zMaster,pPager.dbSize);
			/* If no database changes have been made, return early. */if(pPager.eState<PAGER_WRITER_CACHEMOD)
				return SQLITE_OK;
			if(
			#if SQLITE_OMIT_MEMORYDB
												0 != MEMDB
        ...
        */pPager.pBackup.sqlite3BackupRestart();
            ...
            ** Ticket [2d1a5c67dfc2363e44f29d9bbd57f] 2null11-null5-18 */rc=sqlite3PagerGet(pPager,1,ref pPageOne);
						pList=pPageOne;
						pList.pDirty=null;
					}
					Debug.Assert(rc==SQLITE_OK);
...
rc = pager_incr_changecounter(pPager, 0);
          ...
          */
					...
					if(pPager.dbSize<pPager.dbOrigSize&&pPager.journalMode!=PAGER_JOURNALMODE_OFF) {
						Pgno i;
						/* Iterator variable */Pgno iSkip=PAGER_MJ_PGNO(pPager);
						/* Pending lock page */Pgno dbSize=pPager.dbSize;
						/* Database image size */pPager.dbSize=pPager.dbOrigSize;
						for(i=dbSize+1;i<=pPager.dbOrigSize;i++) {
							if(0==sqlite3BitvecTest(pPager.pInJournal,i)&&i!=iSkip) {
								PgHdr pPage=null;
								/* Page to journal */rc=sqlite3PagerGet(pPager,i,ref pPage);
								if(rc!=SQLITE_OK)
									goto commit_phase_one_exit;
...
*/rc=writeMasterJournal(pPager,zMaster);
          ...
          */if(pPager.dbSize!=pPager.dbFileSize) {
					...
					/* Finally, sync the database file. */if(!noSync) {
						rc=sqlite3PagerSync(pPager);
					}
					IOTRACE("DBSYNC %p\n",pPager);
				}
		...
		static int sqlite3PagerCommitPhaseTwo(Pager pPager) {
			int rc=SQLITE_OK;
			/* Return code *//* This routine should not be called if a prior error has occurred.
      ** But if (due to a coding error elsewhere in the system) it does get
      ** called, just return the same error code without doing anything. */if(NEVER(pPager.errCode)!=0)
				return pPager.errCode;
			Debug.Assert(pPager.eState==PAGER_WRITER_LOCKED||pPager.eState==PAGER_WRITER_FINISHED||(pagerUseWal(pPager)&&pPager.eState==PAGER_WRITER_CACHEMOD));
			Debug.Assert(assert_pager_state(pPager));
      ...
      */if(pPager.eState==PAGER_WRITER_LOCKED&&pPager.exclusiveMode&&pPager.journalMode==PAGER_JOURNALMODE_PERSIST) {
		...
		static int sqlite3PagerRollback(Pager pPager) {
			int rc=SQLITE_OK;
			/* Return code */PAGERTRACE("ROLLBACK %d\n",PAGERID(pPager));
      ...
      */return pager_error(pPager,rc);
		...
		static int sqlite3PagerMemUsed(Pager pPager) {
			...
			return perPageSize*sqlite3PcachePagecount(pPager.pPCache)+0// Not readily available under C#// sqlite3MallocSize(pPager);
			+pPager.pageSize;
		}
		///<summary>
    ...
    static int[] sqlite3PagerStats( Pager pPager )
    {
      ...
      return a;
		...
		static int sqlite3PagerOpenSavepoint(Pager pPager,int nSavepoint) {
			int rc=SQLITE_OK;
			/* Return code */int nCurrent=pPager.nSavepoint;
			/* Current number of savepoints */Debug.Assert(pPager.eState>=PAGER_WRITER_LOCKED);
			Debug.Assert(assert_pager_state(pPager));
			if(nSavepoint>nCurrent&&pPager.useJournal!=0) {
				int ii;
				/* Iterator variable */PagerSavepoint[] aNew;
				/* New Pager.aSavepoint array *//* Grow the Pager.aSavepoint array using realloc(). Return SQLITE_NOMEM
        ...
        *///aNew = (PagerSavepoint *)sqlite3Realloc(
				...
				/* Populate the PagerSavepoint structures just allocated. */for(ii=nCurrent;ii<nSavepoint;ii++) {
		...
		static int sqlite3PagerSavepoint(Pager pPager,int op,int iSavepoint) {
			...
			/* Return code */Debug.Assert(op==SAVEPOINT_RELEASE||op==SAVEPOINT_ROLLBACK);
			Debug.Assert(iSavepoint>=0||op==SAVEPOINT_ROLLBACK);
			if(rc==SQLITE_OK&&iSavepoint<pPager.nSavepoint) {
				int ii;
				/* Iterator variable */int nNew;
				/* Number of remaining savepoints after this op. *//* Figure out how many savepoints will still be active after this
        ...
        ** the sub-journal to zero bytes in size. */if(op==SAVEPOINT_RELEASE) {
					if(nNew==0&&isOpen(pPager.sjfd)) {
						/* Only truncate if it is an in-memory sub-journal. */if(sqlite3IsMemJournal(pPager.sjfd)) {
...
*/static void sqlite3PagerSetCodec(Pager pPager,dxCodec xCodec,//void *(*xCodec)(void*,void*,Pgno,int),
		...
		static int sqlite3PagerMovepage(Pager pPager,DbPage pPg,u32 pgno,int isCommit) {
			PgHdr pPgOld;
			/* The page being overwritten. */u32 needSyncPgno=0;
			/* Old value of pPg.pgno, if sync is required */int rc;
			/* Return code */Pgno origPgno;
			/* The original page number */Debug.Assert(pPg.nRef>0);
			Debug.Assert(pPager.eState==PAGER_WRITER_CACHEMOD||pPager.eState==PAGER_WRITER_DBMOD);
			Debug.Assert(assert_pager_state(pPager));
      ...
      */if((pPg.flags&PGHDR_DIRTY)!=0&&subjRequiresPage(pPg)&&SQLITE_OK!=(rc=subjournalPage(pPg))) {
				return rc;
			}
      ...
      */pPg.flags&=~PGHDR_NEED_SYNC;
          ...
          ** need to rollback later.  Just move the page out of the way. */sqlite3PcacheMove(pPgOld,pPager.dbSize+1);
				}
				else {
...
** only.  We invoke it once here to suppress a compiler warning. */
...
#endif
			/* The eMode parameter is always valid */Debug.Assert(eMode==PAGER_JOURNALMODE_DELETE||eMode==PAGER_JOURNALMODE_TRUNCATE||eMode==PAGER_JOURNALMODE_PERSIST||eMode==PAGER_JOURNALMODE_OFF||eMode==PAGER_JOURNALMODE_WAL||eMode==PAGER_JOURNALMODE_MEMORY);
      ...
      */if(
			...
			if(eMode!=eOld) {
				/* Change the journal mode. */Debug.Assert(pPager.eState!=PAGER_ERROR);
