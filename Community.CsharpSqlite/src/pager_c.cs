...
assert( (pPg->flags&PGHDR.DIRTY) || pPg->pageHash==pager_pagehash(pPg) );
}
...
///</summary>
    static Pgno sqlite3PagerPagenumber( DbPage pPg )
    {
      return pPg.pgno;
        ...
        private static void PAGERTRACE(string p1, int p2, Pgno p3)
        {
            //Sqlite3.PAGETRACE(p1,p2,p3);
        }
        ...
        static SqlResult pagerStress(object p, PgHdr pPg)
        {
            ...
            if (pPager.doNotSpill != 0)
                return SqlResult.SQLITE_OK;
            if (pPager.doNotSyncSpill != 0 && (pPg.flags & PGHDR.NEED_SYNC) != 0)
            {
                return SqlResult.SQLITE_OK;
            }
            pPg.pDirty = null;
            ...
            else
            {
                ...
                if ((pPg.flags & PGHDR.NEED_SYNC) != 0 || pPager.eState == PagerState.PAGER_WRITER_CACHEMOD)
                {
                ...
                if (rc == SqlResult.SQLITE_OK)
                {
                    Debug.Assert((pPg.flags & PGHDR.NEED_SYNC) == 0);
...
szPageDflt = ii;
            ...
            else
            {
                ...
                pPager.eLock = LockType.EXCLUSIVE_LOCK;
                readOnly = (vfsFlags & Sqlite3.SQLITE_OPEN_READONLY) != 0;
            }
            ///
            ///<summary>
            ...
            pPager.syncFlags = (byte)(pPager.noSync ? 0 : Sqlite3.SQLITE_SYNC_NORMAL);
        ...
        static SqlResult pager_write(PgHdr pPg)
        {
            ...
            else
            {
                ...
                if (!pPg.pageInJournal() && !pPager.pagerUseWal())
                {
                    ...
                    if (pPg.pgno <= pPager.dbOrigSize && pPager.jfd.isOpen)
                    {
                        ...
                        Debug.Assert(pPg.pgno != ((Sqlite3.PENDING_BYTE / (pPager.pageSize)) + 1));
            ...
            sqlite3_pager_writej_count.iValue = iValue;
#endif
#endif
                        Sqlite3.PAGERTRACE("JOURNAL %d page %d needSync=%d hash(%08x)\n", PagerMethods.PAGERID(pPager), pPg.pgno, ((pPg.flags & PGHDR.NEED_SYNC) != 0 ? 1 : 0), pPg.pager_pagehash());
                    ...
                    else
                    {
                        ...
                        PAGERTRACE("APPEND %d page %d needSync=%d\n", PagerMethods.PAGERID(pPager), pPg.pgno, ((pPg.flags & PGHDR.NEED_SYNC) != 0 ? 1 : 0));
                    }
                }
                ///
                ///<summary>
...
0 == pPager.memDb
                ...
                ///
                ///<summary>
                ///If the PGHDR.NEED_SYNC flag is set for any of the nPage pages
