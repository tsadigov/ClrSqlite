...
return (pPager->pWal!=0);
			...
			public bool assert_pager_state() {
				Pager pPager=this;
				/* State must be valid. */Debug.Assert(this.eState==PAGER_OPEN||this.eState==PAGER_READER||this.eState==PAGER_WRITER_LOCKED||this.eState==PAGER_WRITER_CACHEMOD||this.eState==PAGER_WRITER_DBMOD||this.eState==PAGER_WRITER_FINISHED||this.eState==PAGER_ERROR);
      ...
      */Debug.Assert(pPager.changeCountDone==false||pPager.eLock>=RESERVED_LOCK);
          ...
          */Debug.Assert(pPager.errCode!=SQLITE_OK);
			...
			int zeroJournalHdr(int doTruncate) {
				int rc=SQLITE_OK;
				/* Return code */Debug.Assert(isOpen(this.jfd));
        ...
        */if(rc==SQLITE_OK&&iLimit>0) {
			...
			int writeJournalHdr() {
				int rc=SQLITE_OK;
				/* Return code */byte[] zHeader=this.pTmpSpace;
				/* Temporary space used to build header */u32 nHeader=(u32)this.pageSize;
				/* Size of buffer pointed to by zHeader */u32 nWrite;
				/* Bytes of header sector written */int ii;
				/* Loop counter */Debug.Assert(isOpen(this.jfd));
				/* Journal file must be open. */if(nHeader>this.JOURNAL_HDR_SZ()) {
      ...
      */Debug.Assert(isOpen(this.fd)||this.noSync);
				...
				/* The initial database size */put32bits(zHeader,aJournalMagic.Length+8,this.dbOrigSize);
				/* The assumed sector size for this process */put32bits(zHeader,aJournalMagic.Length+12,this.sectorSize);
				/* The page size */put32bits(zHeader,aJournalMagic.Length+16,(u32)this.pageSize);
      ...
      */for(nWrite=0;rc==SQLITE_OK&&nWrite<this.JOURNAL_HDR_SZ();nWrite+=nHeader) {
			...
			///
			///</summary>
			int readJournalHdr(/* Pager object */int isHot,i64 journalSize,/* Size of the open journal file in bytes */out u32 pNRec,/* OUT: Value read from the nRec field */out u32 pDbSize/* OUT: Value of original database size field */) {
				int rc;
				/* Return code */byte[] aMagic=new byte[8];
				/* A buffer to hold the magic header */i64 iHdrOff;
				/* Offset of journal header being read */Debug.Assert(isOpen(this.jfd));
      ...
      */if(SQLITE_OK!=(rc=read32bits(this.jfd,iHdrOff+8,ref pNRec))||SQLITE_OK!=(rc=read32bits(this.jfd,iHdrOff+12,ref this.cksumInit))||SQLITE_OK!=(rc=read32bits(this.jfd,iHdrOff+16,ref pDbSize))) {
				...
				if(this.journalOff==0) {
					...
					/* Sector-size field of journal header *//* Read the page-size and sector-size journal header fields. */if(SQLITE_OK!=(rc=read32bits(this.jfd,iHdrOff+20,ref iSectorSize))||SQLITE_OK!=(rc=read32bits(this.jfd,iHdrOff+24,ref iPageSize))) {
						return rc;
					}
        ...
        */this.sectorSize=iSectorSize;
			...
			int writeMasterJournal(string zMaster) {
				...
				/* Checksum of string zMaster */Debug.Assert(this.setMaster==0);
      ...
      */sqlite3PcacheFetch(this.pPCache,pgno,0,ref p);
			...
			int addToSavepointBitvecs(u32 pgno) {
				...
				/* Result code */for(ii=0;ii<this.nSavepoint;ii++) {
			...
			void releaseAllSavepoints() {
				int ii;
				/* Iterator for looping through Pager.aSavepoint */for(ii=0;ii<this.nSavepoint;ii++) {
			...
			void pager_unlock() {
				...
				if(this.pagerUseWal()) {
					...
					if(!this.exclusiveMode) {
						int rc;
						/* Error code returned by pagerUnlockDb() */int iDc=isOpen(this.fd)?sqlite3OsDeviceCharacteristics(this.fd):0;
      ...
      */Debug.Assert(this.assert_pager_state());
				...
				if(isOpen(this.jfd)) {
					Debug.Assert(!this.pagerUseWal());
					/* Finalize the journal file. */if(sqlite3IsMemJournal(this.jfd)) {
...
sqlite3PagerUnref(p);
        ...
        */rc2=sqlite3WalEndWriteTransaction(this.pWal);
			...
			public u32 pager_cksum(byte[] aData) {
				...
				/* Checksum value to return */int i=this.pageSize-200;
			...
			///</summary>
			int pager_playback_one_page(/* The pager being played back */ref i64 pOffset,/* Offset of record to playback */Bitvec pDone,/* Bitvec of pages already played back */int isMainJrnl,/* True for main rollback journal. False for Stmt jrnl */int isSavepnt/* True for a savepoint rollback */) {
				int rc;
				PgHdr pPg;
				...
				/* pDone never used on non-savepoint */aData=this.pTmpSpace;
				Debug.Assert(aData!=null);
				/* Temp storage must have already been allocated */Debug.Assert(this.pagerUseWal()==false||(0==isMainJrnl&&isSavepnt!=0));
      ...
      */if(this.pagerUseWal()) {
        ...
        ** Do this before any decoding. */if(pgno==1) {
					...
					/* Decode the page just read from disk */if(CODEC1(this,pData,pPg.pgno,SQLITE_DECRYPT))
						rc=SQLITE_NOMEM;
      ...
      *///rc = sqlite3OsFileSize(pMaster, &nMasterJournal);
			...
			int pager_truncate(u32 nPage) {
				...
				if(isOpen(this.fd)&&(this.eState>=PAGER_WRITER_DBMOD||this.eState==PAGER_OPEN)) {
					...
					Debug.Assert(this.eLock==EXCLUSIVE_LOCK);
					/* TODO: Is it safe to use Pager.dbFileSize here? */rc=sqlite3OsFileSize(this.fd,ref currentSize);
      ...
      */if(nPage==0) {
					i64 n=0;
					/* Size of db file in bytes */Debug.Assert(isOpen(this.fd)||this.tempFile);
					if(isOpen(this.fd)) {
      ...
      */if(nPage>this.mxPgno) {
			...
			int pagerPlaybackSavepoint(PagerSavepoint pSavepoint) {
				...
				/* Bitvec to ensure pages played back only once */Debug.Assert(this.eState!=PAGER_ERROR);
      ...
      */if(pSavepoint!=null) {
					u32 ii;
					/* Loop counter */i64 offset=pSavepoint.iSubRec*(4+this.pageSize);
					if(this.pagerUseWal()) {
			...
			void sqlite3PagerSetSafetyLevel(/* The pager to set safety level for */int level,/* PRAGMA synchronous.  1=OFF, 2=NORMAL, 3=FULL */int bFullFsync,/* PRAGMA fullfsync */int bCkptFullFsync/* PRAGMA checkpoint_fullfsync */) {
			...
			///</summary>
			int pagerOpentemp(/* The pager object */ref sqlite3_file pFile,/* Write the file descriptor here */int vfsFlags/* Flags passed through to the VFS */) {
				int rc;
				/* Return code */
				#if SQLITE_TEST
																																										#if !TCLSH
			...
			///
			///</summary>
			void sqlite3PagerSetBusyhandler(/* Pager object */dxBusyHandler xBusyHandler,/* Pointer to busy-handler function *///int (*xBusyHandler)(void *),
      ...
      */u32 pageSize=pPageSize;
			...
			public Pgno sqlite3PagerMaxPageCount(int mxPage) {
				...
				Debug.Assert(this.eState!=PAGER_OPEN);
				/* Called only by OP_MaxPgcnt */Debug.Assert(this.mxPgno>=this.dbSize);
				/* OP_MaxPgcnt enforces this */return this.mxPgno;
			}
			public///<summary>
...
pPager.pWal = 0;
      ...
      */sqlite3PcacheClearSyncFlags(this.pPCache);
			...
			int pager_write_pagelist(PgHdr pList) {
				int rc=SQLITE_OK;
				/* Return code *//* This function is only called for rollback pagers in WRITER_DBMOD state. */Debug.Assert(!this.pagerUseWal());
      ...
      */Debug.Assert(rc!=SQLITE_OK||isOpen(this.fd));
        ...
        */if(pList.pgno<=this.dbSize&&0==(pList.flags&PGHDR_DONT_WRITE)) {
						...
						/* Encode the database */if(CODEC2(this,pList.pData,pgno,SQLITE_ENCRYPT_WRITE_CTX,ref pData))
							return SQLITE_NOMEM;
						//     CODEC2(pPager, pList.pData, pgno, 6, return SQLITE_NOMEM, pData);
						/* Write out the page data. */rc=sqlite3OsWrite(this.fd,pData,this.pageSize,offset);
          ...
          */if(pgno==1) {
						...
						/* Update any backup objects copying the contents of this pager. */this.pBackup.sqlite3BackupUpdate(pgno,pList.pData);
          ...
          PAGER_INCR( ref pPager.nWrite );
			...
			int hasHotJournal(ref int pExists) {
				...
				/* True if a journal file is present */int jrnlOpen=isOpen(this.jfd)?1:0;
      ...
      */Debug.Assert(sqlite3PcacheRefCount(this.pPCache)==0);
        ...
        */if(this.eLock<=SHARED_LOCK) {
          ...
          */if(!isOpen(this.jfd)) {
							sqlite3_vfs pVfs=this.pVfs;
							int bExists=0;
							/* True if journal file exists */rc=sqlite3OsAccess(pVfs,this.zJournal,SQLITE_ACCESS_EXISTS,ref bExists);
        ...
        */rc=this.pagerOpenWalIfPresent();
			...
			int sqlite3PagerGet(/* The pager open on the database file */u32 pgno,/* Page number to fetch */ref DbPage ppPage/* Write a pointer to the page here */) {
				return this.sqlite3PagerAcquire(pgno,ref ppPage,0);
			}
			public int sqlite3PagerAcquire(/* The pager open on the database file */u32 pgno,/* Page number to fetch */ref DbPage ppPage,/* Write a pointer to the page here */u8 noContent/* Do not bother reading content from disk if true */) {
				int rc;
				PgHdr pPg=null;
				Debug.Assert(this.eState>=PAGER_READER);
				Debug.Assert(this.assert_pager_state());
      ...
      ** Otherwise, request the page from the PCache layer. */if(this.errCode!=SQLITE_OK) {
					rc=this.errCode;
				}
				else {
        ...
        ** the page. Return without further ado.  */Debug.Assert(pgno<=PAGER_MAX_PGNO&&pgno!=PAGER_MJ_PGNO(this));
        ...
        ** number greater than this, or the unused locking-page, is requested. */if(pgno>PAGER_MAX_PGNO||pgno==PAGER_MJ_PGNO(this)) {
            ...
            */sqlite3BeginBenignMalloc();
			...
			int pager_open_journal() {
				int rc=SQLITE_OK;
				/* Return code */sqlite3_vfs pVfs=this.pVfs;
				/* Local cache of vfs pointer */Debug.Assert(this.eState==PAGER_WRITER_LOCKED);
				Debug.Assert(this.assert_pager_state());
				Debug.Assert(this.pInJournal==null);
				/* If already in the error state, this function is a no-op.  But on
      ** the other hand, this routine is never called if we are already in
      ** an error state. */if(NEVER(this.errCode)!=0)
					return this.errCode;
				if(!this.pagerUseWal()&&this.journalMode!=PAGER_JOURNALMODE_OFF) {
					...
					//  return SQLITE_NOMEM;
					//}
					/* Open the journal file if it is not already open. */if(!isOpen(this.jfd)) {
...
pVfs, pPager.zJournal, pPager.jfd, flags, jrnlBufferSize(pPager)
        ...
        */if(rc==SQLITE_OK) {
						/* TODO: Check if all of these are really required. */this.nRec=0;
						this.journalOff=0;
						this.setMaster=0;
						this.journalHdr=0;
						rc=this.writeJournalHdr();
...
int DIRECT_MODE = isDirectMode;
				...
				if(!this.changeCountDone&&this.dbSize>0) {
					PgHdr pPgHdr=null;
					/* Reference to page 1 */Debug.Assert(!this.tempFile&&isOpen(this.fd));
					/* Open page 1 of the file for writing. */rc=this.sqlite3PagerGet(1,ref pPgHdr);
        ...
        */if(!DIRECT_MODE&&ALWAYS(rc==SQLITE_OK)) {
			...
			///
			///</summary>
			int sqlite3PagerCommitPhaseOne(/* Pager object */string zMaster,/* If not NULL, the master journal name */bool noSync/* True to omit the xSync on the db file */) {
				int rc=SQLITE_OK;
				/* Return code */Debug.Assert(this.eState==PAGER_WRITER_LOCKED||this.eState==PAGER_WRITER_CACHEMOD||this.eState==PAGER_WRITER_DBMOD||this.eState==PAGER_ERROR);
				Debug.Assert(this.assert_pager_state());
				/* If a prior error occurred, report that error again. */if(NEVER(this.errCode!=0))
					return this.errCode;
				PAGERTRACE("DATABASE SYNC: File=%s zMaster=%s nSize=%d\n",this.zFilename,zMaster,this.dbSize);
				/* If no database changes have been made, return early. */if(this.eState<PAGER_WRITER_CACHEMOD)
					return SQLITE_OK;
				if(
				#if SQLITE_OMIT_MEMORYDB
																																										0 != MEMDB
        ...
        */this.pBackup.sqlite3BackupRestart();
            ...
            ** Ticket [2d1a5c67dfc2363e44f29d9bbd57f] 2null11-null5-18 */rc=this.sqlite3PagerGet(1,ref pPageOne);
							pList=pPageOne;
							pList.pDirty=null;
						}
						Debug.Assert(rc==SQLITE_OK);
...
rc = pager_incr_changecounter(pPager, 0);
          ...
          */
						...
						if(this.dbSize<this.dbOrigSize&&this.journalMode!=PAGER_JOURNALMODE_OFF) {
							Pgno i;
							/* Iterator variable */Pgno iSkip=PAGER_MJ_PGNO(this);
							/* Pending lock page */Pgno dbSize=this.dbSize;
							/* Database image size */this.dbSize=this.dbOrigSize;
							for(i=dbSize+1;i<=this.dbOrigSize;i++) {
								if(0==sqlite3BitvecTest(this.pInJournal,i)&&i!=iSkip) {
									PgHdr pPage=null;
									/* Page to journal */rc=this.sqlite3PagerGet(i,ref pPage);
									if(rc!=SQLITE_OK)
										goto commit_phase_one_exit;
...
*/rc=this.writeMasterJournal(zMaster);
      ...
      ** called, just return the same error code without doing anything. */if(NEVER(this.errCode)!=0)
					return this.errCode;
      ...
      */return this.pager_error(rc);
			...
			int sqlite3PagerOpenSavepoint(int nSavepoint) {
				int rc=SQLITE_OK;
				/* Return code */int nCurrent=this.nSavepoint;
				/* Current number of savepoints */Debug.Assert(this.eState>=PAGER_WRITER_LOCKED);
				Debug.Assert(this.assert_pager_state());
        ...
        ** the sub-journal to zero bytes in size. */if(op==SAVEPOINT_RELEASE) {
						if(nNew==0&&isOpen(this.sjfd)) {
							/* Only truncate if it is an in-memory sub-journal. */if(sqlite3IsMemJournal(this.sjfd)) {
      ...
      */pPg.flags&=~PGHDR_NEED_SYNC;
          ...
          ** need to rollback later.  Just move the page out of the way. */sqlite3PcacheMove(pPgOld,this.dbSize+1);
					}
					else {
...
** only.  We invoke it once here to suppress a compiler warning. */
      ...
      */if(
				...
				if(eMode!=eOld) {
					/* Change the journal mode. */Debug.Assert(this.eState!=PAGER_ERROR);
        ...
        */Debug.Assert((PAGER_JOURNALMODE_TRUNCATE&5)==1);
          ...
          */sqlite3OsClose(this.jfd);
				...
				/* Return the new journal mode */return (int)this.journalMode;
			}
			public///<summary>
...
///</summary>
    static Pgno sqlite3PagerPagenumber( DbPage pPg )
    {
      ...
      */if(NEVER(pPager.errCode!=0))
			...
			pPg.pDirty=null;
			if(pPager.pagerUseWal()) {
				/* Write a single frame for this page to the log. */if(pPg.subjRequiresPage()) {
