...
namespace Community.CsharpSqlite {
	...
	public partial class Sqlite3 {
		...
		public class DateTime {
			...
			/* The julian day number times 86400000 */public int Y,M,D;
			/* Year, month, and day */public int h,m;
			/* Hour and minutes */public int tz;
			/* Timezone offset in minutes */public double s;
			/* Seconds */public byte validYMD;
			/* True (1) if Y,M,D are valid */public byte validHMS;
			/* True (1) if h,m,s are valid */public byte validJD;
			///<summary>
			///True (1) if iJD is valid
			///</summary>
			public byte validTZ;
			/* True (1) if tz is valid */public void CopyTo(DateTime ct) {
				ct.iJD=iJD;
				ct.Y=Y;
				ct.M=M;
				ct.D=D;
		...
		static void computeJD(DateTime p) {
			...
			if(p.validYMD!=0) {
				...
				Y=2000;
				/* If no YMD specified, assume 2000-Jan-01 */M=1;
				D=1;
			}
			if(M<=2) {
				Y--;
    ...
    */static void clearYMD_HMS_TZ(DateTime p) {
		...
		///</summary>
		static sqlite3_int64 localtimeOffset(DateTime p,/* Date at which to calculate offset */sqlite3_context pCtx,/* Write error here if one occurs */out int pRc/* OUT: Error code. SQLITE_OK or ERROR */) {
			DateTime x;
			DateTime y=new DateTime();
			time_t t;
			tm sLocal=new tm();
			/* Initialize the contents of sLocal to avoid a compiler warning. *///memset(&sLocal, 0, sizeof(sLocal));
			x=p;
			computeYMD_HMS(x);
		...
		static void strftimeFunc(sqlite3_context context,int argc,sqlite3_value[] argv) {
			{
				...
				for(i=j=0;i<zFmt.Length;i++) {
					if(zFmt[i]!='%') {
						...
						/* Fall thru */case 'j': {
							int nDay;
							/* Number of days since 1st day of year */DateTime y=new DateTime();
							x.CopyTo(y);
							y.validJD=0;
							y.M=1;
							y.D=1;
							...
							if(zFmt[i]=='W') {
								int wd;
								/* 0=Monday, 1=Tuesday, ... 6=Sunday */wd=(int)(((x.iJD+43200000)/86400000)%7);
