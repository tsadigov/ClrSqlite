...
namespace Community.CsharpSqlite {
	...
	public partial class Sqlite3 {
		...
		///
		///</summary>
		static void resolveAlias(Parse pParse,/* Parsing context */ExprList pEList,/* A result set */int iCol,/* A column in the result set.  0..pEList.nExpr-1 */Expr pExpr,/* Transform this into an alias to the result set */string zType/* "GROUP" or "ORDER" or "" */) {
			Expr pOrig;
			/* The iCol-th column of the result set */Expr pDup;
			/* Copy of pOrig */sqlite3 db;
			/* The database connection */Debug.Assert(iCol>=0&&iCol<pEList.nExpr);
			...
			if(pOrig.op!=TK_COLUMN&&(zType.Length==0||zType[0]!='G')) {
				...
				if(ExprHasProperty(pOrig,EP_IntValue)||pOrig.u.zToken==null) {
					pDup=sqlite3ExprDup(db,pOrig,0);
					if(pDup==null)
						return;
				}
      ...
      */ExprSetProperty(pExpr,EP_Static);
		...
		///
		///</summary>
		static int lookupName(Parse pParse,/* The parsing context */string zDb,/* Name of the database containing table, or NULL */string zTab,/* Name of table containing column, or NULL */string zCol,/* Name of the column. */NameContext pNC,/* The name context used to resolve the name */Expr pExpr/* Make this EXPR node point to the selected column */) {
			int i,j;
			/* Loop counters */int cnt=0;
			/* Number of matching column names */int cntTab=0;
			/* Number of matching table names */sqlite3 db=pParse.db;
			/* The database connection */SrcList_item pItem;
			/* Use for looping over pSrcList items */SrcList_item pMatch=null;
			/* The matching pSrcList item */NameContext pTopNC=pNC;
			/* First namecontext in the list */Schema pSchema=null;
			/* Schema of the expression */int isTrigger=0;
			Debug.Assert(pNC!=null);
			/* the name context cannot be NULL. */Debug.Assert(zCol!=null);
			/* The Z in X.Y.Z cannot be NULL */Debug.Assert(!ExprHasAnyProperty(pExpr,EP_TokenOnly|EP_Reduced));
			/* Initialize the node to no-match */pExpr.iTable=-1;
			pExpr.pTab=null;
			ExprSetIrreducible(pExpr);
			/* Start at the inner-most context and move outward until a match is found */while(pNC!=null&&cnt==0) {
				ExprList pEList;
				...
				if(pSrcList!=null) {
						...
						for(j=0;j<pTab.nCol;j++)//, pCol++ )
						 {
							...
							if(pCol.zName.Equals(zCol,StringComparison.InvariantCultureIgnoreCase)) {
								...
								pMatch=pItem;
								pSchema=pTab.pSchema;
								/* Substitute the rowid (column -1) for the INTEGER PRIMARY KEY */pExpr.iColumn=(short)(j==pTab.iPKey?-1:j);
								if(i<pSrcList.nSrc-1) {
									if((pSrcList.a[i+1].jointype&JT_NATURAL)!=0)// pItem[1].jointype
									 {
										/* If this match occurred in the left table of a natural join,
                    ** then skip the right table to avoid a duplicate match *///pItem++;
										i++;
									}
									else
                    ...
                    ** to avoid a duplicate match there. */int k;
        ...
        */if(cnt==0&&cntTab==1&&sqlite3IsRowid(zCol)) {
					cnt=1;
					pExpr.iColumn=-1;
					/* IMP: R-44911-55124 */pExpr.affinity=SQLITE_AFF_INTEGER;
				}
				/*
      ...
      */sqlite3ExprDelete(db,ref pExpr.pLeft);
        ...
        ** the point where the name matched. */for(;;) {
...
*/case TK_ID: {
        ...
        */case TK_CONST_FUNC:
			case TK_FUNCTION: {
				...
				/* True if no such function exists */bool wrong_num_args=false;
				/* True if wrong number of arguments */bool is_agg=false;
				/* True if is an aggregate function */int auth;
				/* Authorization to use the function */int nId;
				/* Number of characters in function name */string zId;
...
return WRC_Prune;
		...
		///
		///</summary>
		static int resolveOrderByTermToExprList(Parse pParse,/* Parsing context for error messages */Select pSelect,/* The SELECT statement with the ORDER BY clause */Expr pE/* The specific ORDER BY term */) {
			int i=0;
			/* Loop counter */ExprList pEList;
			/* The columns of the result set */NameContext nc;
			/* Name context for resolving pE */sqlite3 db;
			/* Database connection */int rc;
			/* Return code from subprocedures */u8 savedSuppErr;
			/* Saved value of db->suppressErr */Debug.Assert(pE.sqlite3ExprIsInteger(ref i)==0);
      ...
      */for(i=0;i<pEList.nExpr;i++) {
			...
			/* If no match, return 0. */return 0;
		}
		///<summary>
		/// Generate an ORDER BY or GROUP BY term out-of-range error.
		///
		///</summary>
		static void resolveOutOfRangeError(Parse pParse,/* The error context into which to write the error */string zType,/* "ORDER" or "GROUP" */int i,/* The index (1-based) of the term out of range */int mx/* Largest permissible value of i */) {
		...
		///
		///</summary>
		static int resolveCompoundOrderBy(Parse pParse,/* Parsing context.  Leave error messages here */Select pSelect/* The SELECT statement containing the ORDER BY */) {
			int i;
			ExprList pOrderBy;
			ExprList pEList;
			sqlite3 db;
		...
		///
		///</summary>
		static int sqlite3ResolveOrderGroupBy(Parse pParse,/* Parsing context.  Leave error messages here */Select pSelect,/* The SELECT statement containing the clause */ExprList pOrderBy,/* The ORDER BY or GROUP BY clause to be processed */string zType/* "ORDER" or "GROUP" */) {
			int i;
			...
			pEList=pSelect.pEList;
			Debug.Assert(pEList!=null);
			/* sqlite3SelectNew() guarantees this */for(i=0;i<pOrderBy.nExpr;i++)//, pItem++)
			 {
				pItem=pOrderBy.a[i];
				if(pItem.iCol!=0) {
		...
		///
		///</summary>
		static int resolveOrderGroupBy(NameContext pNC,/* The name context of the SELECT statement */Select pSelect,/* The SELECT statement holding pOrderBy */ExprList pOrderBy,/* An ORDER BY or GROUP BY clause to resolve */string zType/* Either "ORDER" or "GROUP", as appropriate */) {
			int i;
			/* Loop counter */int iCol;
			/* Column number */ExprList_item pItem;
			/* A term of the ORDER BY clause */Parse pParse;
			/* Parsing context */int nResult;
			/* Number of terms in the result set */if(pOrderBy==null)
				return 0;
          ...
          ** copy of the iCol-th result-set expression. */pItem.iCol=(u16)iCol;
					continue;
				}
				if(pE.sqlite3ExprIsInteger(ref iCol)!=0) {
					/* The ORDER BY term is an integer constant.  Again, set the column
          ** number so that sqlite3ResolveOrderGroupBy() will convert the
          ** order-by term to a copy of the result-set expression */if(iCol<1) {
				...
				/* Otherwise, treat the ORDER BY term as an ordinary expression */pItem.iCol=0;
		...
		static int resolveSelectStep(Walker pWalker,Select p) {
			...
			/* Context that contains this SELECT */NameContext sNC;
			/* Name context of this SELECT */bool isCompound;
			/* True if p is a compound select */int nCompound;
			/* Number of compound terms processed so far */Parse pParse;
			/* Parsing context */ExprList pEList;
			/* Result set expression list */int i;
			/* Loop counter */ExprList pGroupBy;
			/* The GROUP BY clause */Select pLeftmost;
			/* Left-most of SELECT of a compound */sqlite3 db;
			/* Database connection */Debug.Assert(p!=null);
      ...
      */if((p.selFlags&SelectFlags.Expanded)==0) {
        ...
        */sNC.allowAgg=1;
				sNC.pSrcList=p.pSrc;
				sNC.pNext=pOuterNC;
				/* Resolve names in the result set. */pEList=p.pEList;
				Debug.Assert(pEList!=null);
				for(i=0;i<pEList.nExpr;i++) {
      ...
      */if(isCompound&&resolveCompoundOrderBy(pParse,pLeftmost)!=0) {
		...
		///
		///</summary>
		static int sqlite3ResolveExprNames(NameContext pNC,/* Namespace to resolve expressions in. */ref Expr pExpr/* The expression to be analyzed. */) {
			u8 savedHasAgg;
			Walker w=new Walker();
			if(pExpr==null)
				return 0;
...
pParse.nHeight += pExpr.nHeight;
    ...
    */static void sqlite3ResolveSelectNames(Parse pParse,/* The parser context */Select p,/* The SELECT statement being coded. */NameContext pOuterNC/* Name context for parent SELECT statement */) {
			Walker w=new Walker();
			Debug.Assert(p!=null);
			w.xExprCallback=resolveExprStep;
			w.xSelectCallback=resolveSelectStep;
