...
*/static int sqlite3VdbeList(Vdbe p/* The VDBE */) {
			int nRow;
			/* Stop when row count reaches this */int nSub=0;
			/* Number of sub-vdbes seen so far */SubProgram[] apSub=null;
			/* Array of sub-vdbes */Mem pSub=null;
			/* Memory cell hold array of subprogs */sqlite3 db=p.db;
			/* The database connection */int i;
			/* Loop counter */int rc=SQLITE_OK;
			/* Return code */if(p.pResultSet==null)
      ...
      */nRow=p.nOp;
        ...
        ** cells.  */Debug.Assert(p.nMem>9);
				pSub=p.aMem[9];
				if((pSub.flags&MEM_Blob)!=0) {
					/* On the first call to sqlite3_step(), pSub will hold a NULL.  It is
          ** initialized to a BLOB by the P4_SUBPROGRAM processing logic below */apSub=p.aMem[9]._SubProgram;
          ...
          ** pick up the appropriate opcode. */int j;
						i-=p.nOp;
...
sqlite3IoTrace( "SQL %s\n", z.Trim() );
      ...
      */Debug.Assert(p.nOp>0);
		...
		///
		///</summary>
		static void sqlite3VdbeMakeReady(Vdbe p,/* The VDBE */Parse pParse/* Parsing context */) {
			sqlite3 db;
			/* The database connection */int nVar;
			/* Number of parameters */int nMem;
			/* Number of VM memory registers */int nCursor;
			/* Number of cursors required */int nArg;
			/* Number of arguments in subprograms */int n;
			/* Loop counter *///u8 zCsr;                     /* Memory available for allocation */
			//u8 zEnd;                     /* First byte past allocated memory */
			int nByte;
			/* How much extra memory is needed */Debug.Assert(p!=null);
      ...
      *///zCsr = (u8)&p->aOp[p->nOp];       /* Memory avaliable for allocation */
        ...
        *///do
			...
			if(p.aMem!=null) {
				//p.aMem--;                    /* aMem[] goes from 1..nMem */
				p.nMem=nMem;
				/*       not from 0..nMem-1 */for(n=0;n<=nMem;n++) {
...
*/rc=sqlite3VtabSync(db,ref p.zErrMsg);
      ...
      */for(i=0;rc==SQLITE_OK&&i<db.nDb;i++) {
			...
			/* If there are any write-transactions at all, invoke the commit hook */if(needXcommit&&db.xCommitCallback!=null) {
      ...
      */if(0==StringExtensions.sqlite3Strlen30(sqlite3BtreeGetFilename(db.aDb[0].pBt))||nTrans<=1) {
				for(i=0;rc==SQLITE_OK&&i<db.nDb;i++) {
					Btree pBt=db.aDb[i].pBt;
					if(pBt!=null) {
      ...
      */
				...
				bool needSync=false;
				string zMaster="";
				/* File-name for the master journal */string zMainFile=sqlite3BtreeGetFilename(db.aDb[0].pBt);
				sqlite3_file pMaster=null;
				i64 offset=0;
				int res=0;
				/* Select a master journal file name */do {
					i64 iRandom=0;
				...
				if(rc==SQLITE_OK) {
					/* Open the master journal. */rc=sqlite3OsOpenMalloc(ref pVfs,zMaster,ref pMaster,SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE|SQLITE_OPEN_EXCLUSIVE|SQLITE_OPEN_MASTER_JOURNAL,ref rc);
				}
				if(rc!=SQLITE_OK) {
					sqlite3DbFree(db,ref zMaster);
					return rc;
    ...
    */static int sqlite3VdbeCloseStatement(Vdbe p,int eOp) {
      ...
      *///if ( p.db.mallocFailed != 0 )
			...
			/* No commit or rollback needed if the program never started */if(p.pc>=0) {
				...
				/* Set to true if a 'special' error *//* Lock all btrees used by the statement */sqlite3VdbeEnter(p);
        ...
        */if(p.changeCntOn) {
				...
				/* Release the locks */sqlite3VdbeLeave(p);
			}
...
fclose(out);
    ...
    */static u32 sqlite3VdbeSerialType(Mem pMem,int file_format) {
			...
			if((flags&MEM_Int)!=0) {
				/* Figure out whether to use 1, 2, 4, 6 or 8 bytes. */const i64 MAX_6BYTE=((((i64)0x00008000)<<32)-1);
				i64 i=pMem.u.i;
				u64 u;
				...
				if(i<0) {
					if(i<(-MAX_6BYTE))
						return 6;
					/* Previous test prevents:  u = -(-9223372036854775808) */u=(u64)(-i);
				}
				else {
					u=(u64)i;
				}
...
// define swapMixedEndianFloat(X)  X = floatSwap(X)
		...
		static u32 sqlite3VdbeSerialPut(byte[] buf,int offset,int nBuf,Mem pMem,int file_format) {
			...
			u32 len;
			/* Integer and Real */if(serial_type<=7&&serial_type>0) {
				u64 v;
				u32 i;
				if(serial_type==7) {
			...
			/* String or blob */if(serial_type>=12) {
			...
			/* NULL or constants 0 or 1 */return 0;
		}
		///<summary>
		...
		///
		///</summary>
		static u32 sqlite3VdbeSerialGet(byte[] buf,/* Buffer to deserialize from */int offset,/* Offset into Buffer */u32 serial_type,/* Serial type to deserialize */Mem pMem/* Memory cell to write value into */) {
			switch(serial_type) {
			case 10:
			/* Reserved for future use */case 11:
			/* Reserved for future use */case 0: {
				/* NULL */pMem.flags=MEM_Null;
				pMem.n=0;
				pMem.z=null;
				pMem.zBLOB=null;
				break;
			...
			case 1: {
				/* 1-byte signed integer */pMem.u.i=(sbyte)buf[offset+0];
				pMem.flags=MEM_Int;
				return 1;
			}
			case 2: {
				/* 2-byte signed integer */pMem.u.i=(int)((((sbyte)buf[offset+0])<<8)|buf[offset+1]);
				pMem.flags=MEM_Int;
				return 2;
			}
			case 3: {
				/* 3-byte signed integer */pMem.u.i=(int)((((sbyte)buf[offset+0])<<16)|(buf[offset+1]<<8)|buf[offset+2]);
				pMem.flags=MEM_Int;
				return 3;
			}
			case 4: {
				/* 4-byte signed integer */pMem.u.i=(int)(((sbyte)buf[offset+0]<<24)|(buf[offset+1]<<16)|(buf[offset+2]<<8)|buf[offset+3]);
				pMem.flags=MEM_Int;
				return 4;
			}
			case 5: {
				/* 6-byte signed integer */u64 x=(ulong)((((sbyte)buf[offset+0])<<8)|buf[offset+1]);
...
*/
			...
			case 8:
			/* Integer 0 */case 9: {
				/* Integer 1 */pMem.u.i=serial_type-8;
				pMem.flags=MEM_Int;
				return 0;
			}
			default: {
		...
		static int sqlite3VdbeSerialGet(byte[] buf,/* Buffer to deserialize from */u32 serial_type,/* Serial type to deserialize */Mem pMem/* Memory cell to write value into */) {
			switch(serial_type) {
			case 10:
			/* Reserved for future use */case 11:
			/* Reserved for future use */case 0: {
			...
			case 1: {
				/* 1-byte signed integer */pMem.u.i=(sbyte)buf[0];
				pMem.flags=MEM_Int;
				return 1;
			}
			case 2: {
				/* 2-byte signed integer */pMem.u.i=(int)(((buf[0])<<8)|buf[1]);
				pMem.flags=MEM_Int;
				return 2;
			}
			case 3: {
				/* 3-byte signed integer */pMem.u.i=(int)(((buf[0])<<16)|(buf[1]<<8)|buf[2]);
				pMem.flags=MEM_Int;
				return 3;
			}
			case 4: {
				/* 4-byte signed integer */pMem.u.i=(int)((buf[0]<<24)|(buf[1]<<16)|(buf[2]<<8)|buf[3]);
				pMem.flags=MEM_Int;
				return 4;
			}
			case 5: {
				/* 6-byte signed integer */u64 x=(ulong)(((buf[0])<<8)|buf[1]);
...
*/
		...
		///
		///</summary>
		static UnpackedRecord sqlite3VdbeRecordUnpack(KeyInfo pKeyInfo,/* Information about the record format */int nKey,/* Size of the binary record */byte[] pKey,/* The binary record */UnpackedRecord pSpace,//  char *pSpace,          /* Unaligned space available to hold the object */
		int szSpace/* Size of pSpace[] in bytes */) {
			byte[] aKey=pKey;
			UnpackedRecord p;
			/* The unpacked record that we will return */int nByte;
			/* Memory space needed to hold p, in bytes */int d;
			u32 idx;
			int u;
			/* Unsigned loop counter */int szHdr=0;
			Mem pMem;
			int nOff;
      ...
      *///nOff = ( 8 - ( SQLITE_PTR_TO_INT( pSpace ) & 7 ) ) & 7;
			...
			idx=(u32)getVarint32(aKey,0,out szHdr);
			d=(int)szHdr;
			u=0;
			while(idx<(int)szHdr&&u<p.nField&&d<=nKey) {
				...
				pMem.db=pKeyInfo.db;
				/* pMem->flags = 0; // sqlite3VdbeSerialGet() will set this for us *///pMem.zMalloc = null;
    ...
    */static void sqlite3VdbeDeleteUnpackedRecord(UnpackedRecord p) {
      ...
      //}
		...
		static int sqlite3VdbeRecordCompare(int nKey1,byte[] pKey1,/* Left key */UnpackedRecord pPKey2/* Right key */) {
			return sqlite3VdbeRecordCompare(nKey1,pKey1,0,pPKey2);
		}
		static int sqlite3VdbeRecordCompare(int nKey1,byte[] pKey1,/* Left key */int offset,UnpackedRecord pPKey2/* Right key */) {
			int d1;
			/* Offset into aKey[] of next data element */u32 idx1;
			/* Offset into aKey[] of next header element */u32 szHdr1;
			/* Number of bytes in header */int i=0;
			int nField;
			int rc=0;
			...
			mem1.db=pKeyInfo.db;
			/* mem1.flags = 0;  // Will be initialized by sqlite3VdbeSerialGet() *///  VVA_ONLY( mem1.zMalloc = 0; ) /* Only needed by Debug.Assert() statements */
      ...
      *//*  mem1.u.i = 0;  // not needed, here to silence compiler warning */idx1=(u32)((szHdr1=pKey1[offset])<=0x7f?1:getVarint32(pKey1,offset,out szHdr1));
			...
			while(idx1<szHdr1&&i<pPKey2.nField) {
				u32 serial_type1;
				/* Read the serial types for the next element in each key. */idx1+=(u32)((serial_type1=pKey1[offset+idx1])<=0x7f?1:getVarint32(pKey1,(uint)(offset+idx1),out serial_type1));
        ...
        */rc=sqlite3MemCompare(mem1,pPKey2.aMem[i],i<nField?pKeyInfo.aColl[i]:null);
				if(rc!=0) {
					...
					/* Invert the result if we are using DESC sort order. */if(pKeyInfo.aSortOrder!=null&&i<nField&&pKeyInfo.aSortOrder[i]!=0) {
						rc=-rc;
					}
      ...
      */Debug.Assert(rc==0);
			if((pPKey2.flags&UNPACKED_INCRKEY)!=0) {
				...
				if((pPKey2.flags&UNPACKED_PREFIX_MATCH)!=0) {
					/* Leave rc==0 */}
				else
					if(idx1<szHdr1) {
						rc=1;
		...
		static int sqlite3VdbeIdxRowid(sqlite3 db,BtCursor pCur,ref i64 rowid) {
			...
			int rc;
			u32 szHdr=0;
			/* Size of the header */u32 typeRowid=0;
			/* Serial type of the rowid */u32 lenRowid;
			/* Size of the rowid */Mem m=null;
			Mem v=null;
			v=sqlite3Malloc(v);
			UNUSED_PARAMETER(db);
      ...
      */Debug.Assert(sqlite3BtreeCursorIsValid(pCur));
			...
			/* pCur is always valid so KeySize cannot fail */Debug.Assert(((u32)nCellKey&SQLITE_MAX_U32)==(u64)nCellKey);
			/* Read in the complete content of the index entry */m=sqlite3Malloc(m);
			// memset(&m, 0, sizeof(m));
			rc=sqlite3VdbeMemFromBtree(pCur,0,(int)nCellKey,true,m);
			if(rc!=0) {
				return rc;
			}
			/* The index entry must begin with a header size */getVarint32(m.zBLOB,0,out szHdr);
			testcase(szHdr==3);
			testcase(szHdr==m.n);
      ...
      ** Verify that the last entry really is an integer. */getVarint32(m.zBLOB,szHdr-1,out typeRowid);
			...
			/* Fetch the integer off the end of the index record */sqlite3VdbeSerialGet(m.zBLOB,(int)(m.n-lenRowid),typeRowid,v);
			rowid=v.u.i;
			sqlite3VdbeMemRelease(m);
			return SQLITE_OK;
			/* Jump here if database corruption is detected after m has been
    ** allocated.  Free the m object and return SQLITE_CORRUPT. */idx_rowid_corruption:
		...
		///
		///</summary>
		static int sqlite3VdbeIdxKeyCompare(VdbeCursor pC,/* The cursor to compare against */UnpackedRecord pUnpacked,/* Unpacked version of key to compare against */ref int res/* Write the comparison result here */) {
			i64 nCellKey=0;
			int rc;
			...
			rc=sqlite3BtreeKeySize(pCur,ref nCellKey);
			Debug.Assert(rc==SQLITE_OK);
			/* pCur is always valid so KeySize cannot fail *//* nCellKey will always be between 0 and 0xffffffff because of the say
      ** that btreeParseCellPtr() and sqlite3GetVarint32() are implemented */if(nCellKey<=0||nCellKey>0x7fffffff) {
				res=0;
				return SQLITE_CORRUPT_BKPT();
			}
			m=sqlite3Malloc(m);
