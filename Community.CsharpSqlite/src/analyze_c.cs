...
namespace Community.CsharpSqlite {
	...
	public partial class Sqlite3 {
		...
		static void openStatTable(Parse pParse,/* Parsing context */int iDb,/* The database we are looking in */int iStatCur,/* Open the sqlite_stat1 table on this cursor */string zWhere,/* Delete entries for this table or index */string zWhereType/* Either "tbl" or "idx" */) {
			int[] aRoot=new int[] {
				0,
				0
			};
      ...
      ** because the OpenWrite opcode below will be needing it. */sqlite3NestedParse(pParse,"CREATE TABLE %Q.%s(%s)",pDb.zName,zTab,aTable[i].zCols);
      ...
      ** entire contents of the table. */aRoot[i]=pStat.tnum;
		...
		/// a single table.
		///</summary>
		static void analyzeOneTable(Parse pParse,/* Parser context */Table pTab,/* Table whose indices are to be analyzed */Index pOnlyIdx,/* If not NULL, only analyze this one index */int iStatCur,/* Index of VdbeCursor that writes the sqlite_stat1 table */int iMem/* Available memory locations begin here */) {
			sqlite3 db=pParse.db;
...
#endif
			/* Establish a read-lock on the table at the shared-cache level. */sqlite3TableLock(pParse,iDb,pTab.tnum,0,pTab.zName);
			iIdxCur=pParse.nTab++;
...
*/static void analyzeTable(Parse pParse,Table pTab,Index pOnlyIdx) {
  ...
  ** and code in pParse and return NULL. */Debug.Assert(sqlite3BtreeHoldsAllMutexes(pParse.db));
			...
			if(pName1==null) {
				/* Form 1:  Analyze everything */for(i=0;i<db.nDb;i++) {
					if(i==1)
						continue;
					/* Do not analyze the TEMP database */analyzeDatabase(pParse,i);
				}
			}
			else
				if(pName2.n==0) {
					...
					if(iDb>=0) {
						...
						if(z!=null) {
							if((pIdx=sqlite3FindIndex(db,z,null))!=null) {
								...
								if((pTab=sqlite3LocateTable(pParse,0,z,null))!=null) {
					...
					/* Form 3: Analyze the fully qualified table name */iDb=sqlite3TwoPartName(pParse,pName1,pName2,ref pTableName);
					if(iDb>=0) {
						...
						if(z!=null) {
							if((pIdx=sqlite3FindIndex(db,z,zDb))!=null) {
								...
								if((pTab=sqlite3LocateTable(pParse,0,z,zDb))!=null) {
									analyzeTable(pParse,pTab,null);
								}
							z=null;
							//sqlite3DbFree( db, z );
