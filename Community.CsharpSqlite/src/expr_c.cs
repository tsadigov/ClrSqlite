	...
	public partial class Sqlite3 {
        ...
        */return SQLITE_AFF_NONE;
		...
		static int codeCompare(Parse pParse,/* The parsing (and code generating) context */Expr pLeft,/* The left operand */Expr pRight,/* The right operand */int opcode,/* The comparison opcode */int in1,int in2,/* Register holding operands */int dest,/* Jump here if true.  */int jumpIfNull/* If true, jump if either operand is NULL */) {
			...
			addr=sqlite3VdbeAddOp4(pParse.pVdbe,opcode,in2,dest,in1,p4,P4_COLLSEQ);
...
*/static void sqlite3ExprCodeIN(Parse pParse,/* Parsing and code generating context */Expr pExpr,/* The IN expression */int destIfFalse,/* Jump here if LHS is not contained in the RHS */int destIfNull/* Jump here if the results are unknown due to NULLs */) {
      ...
      */if(destIfNull==destIfFalse) {
        ...
        ** the same. */sqlite3VdbeAddOp2(v,OP_IsNull,r1,destIfNull);
			}
			else {
			...
			case TK_AGG_COLUMN:
			{
				...
				if(pAggInfo.directMode==0) {
					...
					if(pAggInfo.useSortingIdx!=0) {
						sqlite3VdbeAddOp3(v,OP_Column,pAggInfo.sortingIdx,pCol.iSorterColumn,target);
						break;
					}
...
*/if(nFarg>=2&&(pExpr.flags&EP_InfixFunc)!=0) {
				...
				sqlite3VdbeAddOp4(v,OP_Function,constMask,r1,target,pDef,P4_FUNCDEF);
        ...
        */default: {
			...
			case TK_RAISE: {
				...
				if(pExpr.affinity==OE_Ignore) {
					sqlite3VdbeAddOp4(v,OP_Halt,SQLITE_OK,OE_Ignore,0,pExpr.u.zToken,0);
				}
				else {
			...
			for(i=0;i<n;i++)// pItem++)
			 {
				...
				if(inReg!=target+i) {
					sqlite3VdbeAddOp2(pParse.pVdbe,doHardCopy?OP_Copy:OP_SCopy,inReg,target+i);
				}
			}
			return n;
		}
