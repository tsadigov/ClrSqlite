...
namespace Community.CsharpSqlite {
	...
	public partial class Sqlite3 {
		...
		///
		///</summary>
		static void corruptSchema(InitData pData,/* Initialization context */string zObj,/* Object being parsed at the point of error */string zExtra/* Error information */) {
			sqlite3 db=pData.db;
		...
		static int sqlite3InitCallback(object pInit,sqlite3_int64 argc,object p2,object NotUsed) {
			...
			if(argv==null)
				return 0;
			/* Might happen if EMPTY_RESULT_CALLBACKS are on */if(argv[1]==null) {
				corruptSchema(pData,argv[0],"");
			}
			else
      ...
      */if(OMIT_TEMPDB==0&&iDb==1) {
			...
			/* Construct the schema tables.  */azArg[0]=zMasterName;
			azArg[1]="1";
      ...
      ** will be closed before this function returns.  */sqlite3BtreeEnter(pDb.pBt);
      ...
      */for(i=0;i<ArraySize(meta);i++) {
				pDb.pBt.sqlite3BtreeGetMeta(i+1,ref meta[i]);
			}
      ...
      */if(meta[BTREE_TEXT_ENCODING-1]!=0) {
				/* text encoding */if(iDb==0) {
					SqliteEncoding encoding;
					/* If opening the main database, set ENC(db). */encoding=(SqliteEncoding)(meta[BTREE_TEXT_ENCODING-1]&3);
					if(encoding==0)
					...
					/* If opening an attached database, the encoding much match ENC(db) */if((SqliteEncoding)meta[BTREE_TEXT_ENCODING-1]!=ENC(db)) {
...
*/}
      ...
      */
		...
		static void schemaIsValid(Parse pParse) {
			...
			for(iDb=0;iDb<db.nDb;iDb++) {
				int openedTransaction=0;
				/* True if a transaction is opened */Btree pBt=db.aDb[iDb].pBt;
				/* Btree database to read cookie from */if(pBt==null)
					continue;
				/* If there is not already a read-only (or read-write) transaction opened
        ** on the b-tree database, open one now. If a transaction is opened, it 
        ** will be closed immediately after reading the meta-value. */if(!pBt.sqlite3BtreeIsInReadTrans()) {
        ...
        ** set Parse.rc to SQLITE_SCHEMA. */pBt.sqlite3BtreeGetMeta(BTREE_SCHEMA_VERSION,ref cookie);
				Debug.Assert(sqlite3SchemaMutexHeld(db,iDb,null));
				if(cookie!=db.aDb[iDb].pSchema.schema_cookie) {
					sqlite3ResetInternalSchema(db,iDb);
					pParse.rc=SQLITE_SCHEMA;
				}
				/* Close the transaction, if one was opened. */if(openedTransaction!=0) {
      ...
      */Debug.Assert(sqlite3_mutex_held(db.mutex));
		...
		///
		///</summary>
		static int sqlite3Prepare(sqlite3 db,/* Database handle. */string zSql,/* UTF-8 encoded SQL statement. */int nBytes,/* Length of zSql in bytes. */int saveSqlFlag,/* True to copy SQL text into the sqlite3_stmt */Vdbe pReprepare,/* VM being reprepared */ref sqlite3_stmt ppStmt,/* OUT: A pointer to the prepared statement */ref string pzTail/* OUT: End of parsed string */) {
			Parse pParse;
			/* Parsing context */string zErrMsg="";
			/* Error message */int rc=SQLITE_OK;
			/* Result code */int i;
			/* Loop counter */ppStmt=null;
			pzTail=null;
			/* Allocate the parsing context */pParse=new Parse();
      ...
      */for(i=0;i<db.nDb;i++) {
			...
			//if (pzTail != null)
			{
				...
				#endif
				/* Delete any TriggerPrg structures allocated while parsing this statement. *///sqlite3StackFree( db, pParse );
			}
			rc=pParse.rc;
		...
		static int sqlite3Reprepare(Vdbe p) {
			...
			/* Reprepare only called for prepare_v2() statements */db=sqlite3VdbeDb(p);
			Debug.Assert(sqlite3_mutex_held(db.mutex));
		...
		static public int sqlite3_prepare(sqlite3 db,/* Database handle. */string zSql,/* UTF-8 encoded SQL statement. */int nBytes,/* Length of zSql in bytes. */ref sqlite3_stmt ppStmt,/* OUT: A pointer to the prepared statement */int dummy/* OUT: End of parsed string */) {
			string sOut=null;
		...
		///
		///</summary>
		static public int sqlite3_prepare(sqlite3 db,/* Database handle. */string zSql,/* UTF-8 encoded SQL statement. */int nBytes,/* Length of zSql in bytes. */ref sqlite3_stmt ppStmt,/* OUT: A pointer to the prepared statement */ref string pzTail/* OUT: End of parsed string */) {
			int rc;
		...
		public static int sqlite3_prepare_v2(sqlite3 db,/* Database handle. */string zSql,/* UTF-8 encoded SQL statement. */int nBytes,/* Length of zSql in bytes. */ref sqlite3_stmt ppStmt,/* OUT: A pointer to the prepared statement */int dummy/* ( No string passed) */) {
			string pzTail=null;
			int rc;
		...
		public static int sqlite3_prepare_v2(sqlite3 db,/* Database handle. */string zSql,/* UTF-8 encoded SQL statement. */int nBytes,/* Length of zSql in bytes. */ref sqlite3_stmt ppStmt,/* OUT: A pointer to the prepared statement */ref string pzTail/* OUT: End of parsed string */) {
			int rc;
			rc=sqlite3LockAndPrepare(db,zSql,nBytes,1,null,ref ppStmt,ref pzTail);
			Debug.Assert(rc==SQLITE_OK||ppStmt==null);
			/* VERIFY: F13021 */return rc;
