...
namespace Community.CsharpSqlite {
    ...
    *///#define _SQLITE_OS_C_ 1
    ...
    static void DO_OS_MALLOC_TEST( sqlite3_file x )
    {
		...
		static int sqlite3OsShmMap(sqlite3_file id,/* Database file handle */int iPage,int pgsz,int bExtend,/* True to extend file if necessary */out object pp/* OUT: Pointer to mapping */) {
		...
		///
		///</summary>
		static int sqlite3OsOpen(sqlite3_vfs pVfs,string zPath,sqlite3_file pFile,int flags,ref int pFlagsOut) {
			int rc;
			DO_OS_MALLOC_TEST(null);
			/* 0x87f3f is a mask of SQLITE_OPEN_ flags that are valid to be passed
      ** down into the VFS layer.  Some SQLITE_OPEN_ flags (for example,
      ** SQLITE_OPEN_FULLMUTEX or SQLITE_OPEN_SHAREDCACHE) are blocked before
      ** reaching the VFS. */rc=pVfs.xOpen(pVfs,zPath,pFile,flags&0x87f3f,out pFlagsOut);
		...
		static int sqlite3OsFullPathname(sqlite3_vfs pVfs,string zPath,int nPathOut,StringBuilder zPathOut) {
			zPathOut.Length=0;
			//zPathOut[0] = 0;
      ...
      */if(pVfs.iVersion>=2&&pVfs.xCurrentTimeInt64!=null) {
		...
		static int sqlite3OsOpenMalloc(ref sqlite3_vfs pVfs,string zFile,ref sqlite3_file ppFile,int flags,ref int pOutFlags) {
			int rc=SQLITE_NOMEM;
			sqlite3_file pFile;
