...
namespace Community.CsharpSqlite {
	public partial class Sqlite3 {
		...
		static void sqlite3HashClear(Hash pH) {
			HashElem elem;
			/* For looping over all elements of the table */Debug.Assert(pH!=null);
			elem=pH.first;
			pH.first=null;
		...
		///
		///</summary>
		static void insertElement(Hash pH,/* The complete hash table */_ht pEntry,/* The entry into which pNew is inserted */HashElem pNew/* The element to be inserted */) {
			HashElem pHead;
			/* First element already in pEntry */if(pEntry!=null) {
		...
		static bool rehash(ref Hash pH,u32 new_size) {
			_ht[] new_ht;
			/* The new hash table */HashElem elem;
			HashElem next_elem;
...
*/sqlite3BeginBenignMalloc();
		...
		///
		///</summary>
		static HashElem findElementGivenHash(Hash pH,/* The pH to be searched */string pKey,/* The key we are searching for */int nKey,/* Bytes in key (not counting zero terminator) */u32 h/* The hash for this key. */) {
			HashElem elem;
			/* Used to loop thru the element list */int count;
			/* Number of elements left to test */if(pH.ht!=null&&pH.ht[h]!=null) {
				_ht pEntry=pH.ht[h];
				elem=pEntry.chain;
		...
		///
		///</summary>
		static void removeElementGivenHash(Hash pH,/* The pH containing "elem" */ref HashElem elem,/* The element to be removed from the pH */u32 h/* Hash value for the element */) {
			_ht pEntry;
			if(elem.prev!=null) {
		...
		static T sqlite3HashFind<T>(Hash pH,string pKey,int nKey,T nullType) where T : class {
			HashElem elem;
			/* The element that matches key */u32 h;
			/* A hash on key */Debug.Assert(pH!=null);
    ...
    */static T sqlite3HashInsert<T>(ref Hash pH,string pKey,int nKey,T data) where T : class {
			u32 h;
			/* the hash of the key modulo hash table size */HashElem elem;
			/* Used to loop thru the element list */HashElem new_elem;
			/* New element added to the pH */Debug.Assert(pH!=null);
			Debug.Assert(pKey!=null);
			Debug.Assert(nKey>=0);
			if(pH.htsize!=0) {
				h=strHash(pKey,nKey)%pH.htsize;
