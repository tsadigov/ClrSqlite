      ...
      */if((fg&MEM_Int)!=0) {
		...
		static int sqlite3VdbeMemFinalize(Mem pMem,FuncDef pFunc) {
			...
			if(ALWAYS(pFunc!=null&&pFunc.xFinalize!=null)) {
				...
				ctx.pFunc=pFunc;
				pFunc.xFinalize(ctx);
				/* IMP: R-24505-23230 */Debug.Assert(0==(pMem.flags&MEM_Dyn)&&pMem.xDel==null);
		...
		static void sqlite3VdbeMemReleaseExternal(Mem p) {
			...
			if((p.flags&(MEM_Agg|MEM_Dyn|MEM_RowSet|MEM_Frame))!=0) {
				if((p.flags&MEM_Agg)!=0) {
					...
					if((p.flags&MEM_Dyn)!=0&&p.xDel!=null) {
						Debug.Assert((p.flags&MEM_RowSet)==0);
						p.xDel(ref p.z);
						p.xDel=null;
					}
					else
						if((p.flags&MEM_RowSet)!=0) {
							sqlite3RowSetClear(p.u.pRowSet);
						}
						else
							if((p.flags&MEM_Frame)!=0) {
...
*/const i64 maxInt=IntegerExtensions.LARGEST_INT64;
        ...
        ** does so for compatibility we will do the same in software. */return minInt;
				}
				else {
					return (i64)r;
				}
			...
			if((pMem.flags&MEM_Real)!=0) {
				...
				if((pMem.flags&MEM_Int)!=0) {
					...
					if((pMem.flags&(MEM_Str))!=0) {
						/* (double)0 In case of SQLITE_OMIT_FLOATING_POINT... */double val=(double)0;
						...
						if((pMem.flags&(MEM_Blob))!=0) {
							...
							/* (double)0 In case of SQLITE_OMIT_FLOATING_POINT... */return (double)0;
						}
		}
		///<summary>
      ...
      */if(pMem.r==(double)pMem.u.i&&pMem.u.i>IntegerExtensions.SMALLEST_INT64&&ALWAYS(pMem.u.i<IntegerExtensions.LARGEST_INT64)) {
				pMem.flags|=MEM_Int;
			}
		}
		///<summary>
...
*/
    static void sqlite3VdbeMemPrepareToChange( Vdbe pVdbe, Mem pMem )
    {
      ...
      pMem.pScopyFrom = null;
		...
		///
		///</summary>
		static int sqlite3VdbeMemSetBlob(Mem pMem,/* Memory cell to set to string value */byte[] zBlob,/* Blob pointer */int n,/* Bytes in Blob */SqliteEncoding enc,/* 0 for BLOBs */dxDel xDel/* Destructor function */) {
			return sqlite3VdbeMemSetBlob(pMem,zBlob,0,n>=0?n:zBlob.Length,enc,xDel);
		}
		// Call w/o offset
		static int sqlite3VdbeMemSetBlob(Mem pMem,/* Memory cell to set to string value */byte[] zBlob,/* Blob pointer */int offset,/* offset into string */int n,/* Bytes in string, or negative */SqliteEncoding enc,/* Encoding of z.  0 for BLOBs */dxDel xDel//)(void*)/* Destructor function */
		) {
			int nByte=n;
			/* New value for pMem->n */int iLimit;
			/* Maximum allowed string or blob size */Debug.Assert(pMem.db==null||sqlite3_mutex_held(pMem.db.mutex));
			Debug.Assert((pMem.flags&MEM_RowSet)==0);
			/* If zBlob is a NULL pointer, set pMem to contain an SQL NULL. */if(zBlob==null||zBlob.Length<offset) {
				sqlite3VdbeMemSetNull(pMem);
				return SQLITE_OK;
			}
			if(pMem.db!=null) {
      ...
      */Debug.Assert(enc==0);
		...
		static int sqlite3VdbeMemSetStr(Mem pMem,/* Memory cell to set to string value */string z,/* String pointer */int n,/* Bytes in string, or negative */SqliteEncoding enc,/* Encoding of z.  0 for BLOBs */dxDel xDel/* Destructor function */) {
			return sqlite3VdbeMemSetStr(pMem,z,0,n,enc,xDel);
		}
		// Call w/o offset
		static int sqlite3VdbeMemSetStr(Mem pMem,/* Memory cell to set to string value */string z,/* String pointer */int offset,/* offset into string */int n,/* Bytes in string, or negative */SqliteEncoding enc,/* Encoding of z.  0 for BLOBs */dxDel xDel//)(void*)/* Destructor function */
		) {
			int nByte=n;
			/* New value for pMem->n */int iLimit;
			/* Maximum allowed string or blob size */u16 flags=0;
			/* New value for pMem->flags */Debug.Assert(pMem.db==null||sqlite3_mutex_held(pMem.db.mutex));
			Debug.Assert((pMem.flags&MEM_RowSet)==0);
			/* If z is a NULL pointer, set pMem to contain an SQL NULL. */if(z==null||z.Length<offset) {
				sqlite3VdbeMemSetNull(pMem);
				return SQLITE_OK;
			}
			if(pMem.db!=null) {
...
return SQLITE_NOMEM;
      ...
      */if((combined_flags&MEM_Str)!=0) {
        ...
        */Debug.Assert(pColl==null||pColl.xCmp!=null);
            ...
            ** comparison function directly */return pColl.xCmp(pColl.pUser,pMem1.n,pMem1.z,pMem2.n,pMem2.z);
					}
					else {
						string v1,v2;
						int n1,n2;
        ...
        ** to the blob case and use memcmp().  */}
			/* Both values must be blobs.  Compare using memcmp().  */if((pMem1.flags&MEM_Blob)!=0)
				if(pMem1.zBLOB!=null)
		...
		///
		///</summary>
		static int sqlite3VdbeMemFromBtree(BtCursor pCur,/* Cursor pointing at record to retrieve. */int offset,/* Offset from the start of data to return bytes from. */int amt,/* Number of bytes to return. */bool key,/* If true, retrieve from the btree key, not data. */Mem pMem/* OUT: Return data in this Mem structure. */) {
			byte[] zData;
			/* Data from the btree layer */int available=0;
			/* Number of bytes available on the local btree page */int rc=SQLITE_OK;
			/* Return code */Debug.Assert(sqlite3BtreeCursorIsValid(pCur));
			/* Note: the calls to BtreeKeyFetch() and DataFetch() below assert()
      ** that both the BtShared and database handle mutexes are held. */Debug.Assert((pMem.flags&MEM_RowSet)==0);
			int outOffset=-1;
			if(key) {
		...
		///
		///</summary>
		static int sqlite3ValueFromExpr(sqlite3 db,/* The database connection */Expr pExpr,/* The expression to evaluate */SqliteEncoding enc,/* Encoding to use */char affinity,/* Affinity to use */ref sqlite3_value ppVal/* Write the new value here */) {
			int op;
			string zVal="";
...
*/if(op==TK_UMINUS&&(pExpr.pLeft.op==TK_INTEGER||pExpr.pLeft.op==TK_FLOAT)) {
			...
			if(enc!=SqliteEncoding.UTF8) {
				...
				if(op==TK_UMINUS) {
					/* This branch happens for multiple negative signs.  Ex: -(-5) */if(SQLITE_OK==sqlite3ValueFromExpr(db,pExpr.pLeft,enc,affinity,ref pVal)) {
		...
		///
		///</summary>
		static void sqlite3ValueSetStr(sqlite3_value v,/* Value to be set */int n,/* Length of string z */string z,/* Text of the new string */SqliteEncoding enc,/* Encoding to use */dxDel xDel//)(void*) /* Destructor for the string */
		) {
			if(v!=null)
