...
#endif
			for(opcodeIndex=p.pc;rc==SQLITE_OK;opcodeIndex++) {
        ...
        if ( ( pOp.opflags & OPFLG_OUT3 ) != 0 )
        {
          ...
          */case OpCode.OP_Halt: {
					...
					if(pOp.p1==SQLITE_OK&&p.pFrame!=null) {
						...
						opcodeIndex=sqlite3VdbeFrameRestore(pFrame);
                  ...
                  ** as the p2 of the calling OP_Program.  */opcodeIndex=p.aOp[opcodeIndex].p2-1;
						}
						aOp=p.aOp;
						aMem=p.aMem;
						break;
...
pOp.p1 = pOut.n;
          ...
          */case OpCode.OP_ResultRow: {
              ...
              ** transaction. It needs to be rolled back.  */if(SQLITE_OK!=(rc=sqlite3VdbeCheckFk(p,0))) {
          ...
          */
              ...
              */sqlite3VdbeMemMove(ctx.s,pOut);
					...
					if((ctx.pFunc.flags&SQLITE_FUNC_NEEDCOLL)!=0) {
						...
						Debug.Assert(p.aOp[opcodeIndex-1].p4type==P4_COLLSEQ);
						//Debug.Assert(pOp[-1].p4type == P4_COLLSEQ);
						Debug.Assert(p.aOp[opcodeIndex-1].opcode==OP_CollSeq);
						//Debug.Assert(pOp[-1].opcode == OP_CollSeq);
						ctx.pColl=p.aOp[opcodeIndex-1].p4.pColl;
      ...
      ** that the db->aVTrans[] array is empty.  */Debug.Assert(db.autoCommit==0||db.nVTrans==0);
          ...
          */
                case OpCode.OP_ReadCookie:
					...
					db.aDb[iDb].pBt.sqlite3BtreeGetMeta(iCookie,ref iMeta);
					pOut.u.i=(int)iMeta;
					break;
				}
          ...
          */
                case OpCode.OP_VerifyCookie:
					...
					pBt=db.aDb[pOp.p1].pBt;
					if(pBt!=null) {
						pBt.sqlite3BtreeGetMeta(BTREE_SCHEMA_VERSION,ref iMeta);
...
*/case OpCode.OP_Program: {
              ...
              ** is already allocated. Otherwise, it must be initialized.  */if((pRt.flags&MEM_Frame)==0) {
                ...
                */nMem=pProgram.nMem+pProgram.nCsr;
						...
						Debug.Assert(opcodeIndex==pFrame.pc);
					}
					p.nFrame++;
...
*/case OpCode.OP_IfPos: {
          ...
          */case OpCode.OP_AggStep: {
					...
					if((ctx.pFunc.flags&SQLITE_FUNC_NEEDCOLL)!=0) {
						...
						Debug.Assert(p.aOp[opcodeIndex-1].OpCode==OpCode.OP_CollSeq);
