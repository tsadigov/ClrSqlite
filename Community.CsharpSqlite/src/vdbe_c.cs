        ...
        if ( ( pOp.opflags & OPFLG_OUT3 ) != 0 )
        {
          ...
          */case OP_HaltIfNull: {
					...
					if((pIn3.flags&MEM_Null)==0)
						break;
					/* Fall through into OP_Halt */goto case OP_Halt;
				}
          ...
          */case OP_Halt: {
                  ...
                  ** as the p2 of the calling OP_Program.  */pc=p.aOp[pc].p2-1;
						}
						aOp=p.aOp;
						aMem=p.aMem;
						break;
...
pOp.p1 = pOut.n;
					...
					/* Fall through to the next case, OP_String */goto case OP_String;
				}
          ...
          */case OP_Add:
				/* same as TK_PLUS, in1, in2, ref3 */case OP_Subtract:
				/* same as TK_MINUS, in1, in2, ref3 */case OP_Multiply:
				/* same as TK_STAR, in1, in2, ref3 */case OP_Divide:
				/* same as TK_SLASH, in1, in2, ref3 */case OP_Remainder: {
...
MemSetTypeFlag(pOut, MEM_Int);
  ...
  */
  if( p.expired ) rc = SQLITE_ABORT;
          ...
          ** If either input is NULL, the result is NULL.
          */case OP_BitAnd:
				/* same as TK_BITAND, in1, in2, ref3 */case OP_BitOr:
				/* same as TK_BITOR, in1, in2, ref3 */case OP_ShiftLeft:
				/* same as TK_LSHIFT, in1, in2, ref3 */case OP_ShiftRight: {
...
*//* Opcode: Ne P1 P2 P3 P4 P5
          ...
          ** register P1.  See the Lt opcode for additional information.
          */case OP_Eq:
				/* same as TK_EQ, jump, in1, in3 */case OP_Ne:
				/* same as TK_NE, jump, in1, in3 */case OP_Lt:
				/* same as TK_LT, jump, in1, in3 */case OP_Le:
				/* same as TK_LE, jump, in1, in3 */case OP_Gt:
				/* same as TK_GT, jump, in1, in3 */case OP_Ge: {
          ...
          ** give a NULL output.
          */case OP_And:
				/* same as TK_AND, in1, in2, ref3 */case OP_Or: {
...
*/case OP_Savepoint: {
      ...
      ** that the db->aVTrans[] array is empty.  */Debug.Assert(db.autoCommit==0||db.nVTrans==0);
          ...
          */case OP_SeekLt:
				/* jump, in3 */case OP_SeekLe:
				/* jump, in3 */case OP_SeekGe:
				/* jump, in3 */case OP_SeekGt: {
					/* jump, in3 */int res;
					int oc;
					VdbeCursor pC;
					UnpackedRecord r;
                  ...
                  ** loss of information, then special processing is required... */if((pIn3.flags&MEM_Int)==0) {
                      ...
                      ** then the seek is not possible, so jump to P2 */pc=pOp.p2-1;
									break;
								}
...
** an AUTOINCREMENT table. *//* on the first attempt, simply do one more than previous */v=lastRowid;
          ...
          */case OP_Sort: {
              ...
              sqlite3_search_count.iValue--;
					...
					/* Fall through into OP_Rewind */goto case OP_Rewind;
				}
				/* Opcode: Rewind P1 P2 * * *
          **
          ** The next use of the Rowid or Column or Next instruction for P1
          ...
          */case OP_Prev:
				/* jump */case OP_Next: {
					...
					rc=pOp.opcode==OP_Next?sqlite3BtreeNext(pCrsr,ref res):sqlite3BtreePrevious(pCrsr,ref res);
					pC.nullRow=res==1?true:false;
					pC.cacheStatus=CACHE_STALE;
					if(res==0) {
						pc=pOp.p2-1;
          ...
          */case OP_CreateIndex:
				/* out2-prerelease */case OP_CreateTable: {
...
*/case OP_VNext: {
              ...
              */p.inVtabMethod=1;
					...
					if(0==res) {
						/* If there is data, jump to P2 */pc=pOp.p2-1;
					}
					break;
				}
				#endif
...
*//*
          ...
          */default: {
					/* This is really OP_Noop and OP_Explain */Debug.Assert(pOp.opcode==OP_Noop||pOp.opcode==OP_Explain);
					break;
				}
				/*****************************************************************************
